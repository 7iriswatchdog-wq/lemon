...
//#endif
        ...
        public async Task<ShareholderResults> ScreenShareHolder(CustomerCaseWithShareholderApiModel model, ShareholderDetails shareholder, int shareholderIndex, string body)
        {
            ...
            var apiRespShareHolder = await _commonService.CustomerScreeningCall(_ccDTO, baseURL, c6BaseURL, "API", body, model.Threshold, resultShareholder.Result.ParseInt());
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (result.Result != "")
                    {
                        ...
                        {
                            var corporateScreeningTask = _commonService.CustomerScreeningCall(_ccDTO, baseURL, c6BaseURL, "API", body, model.Threshold, result.Result.ParseInt());
                            ...
                            {
                                var sh = shareholder;
                                taskList.Add(Task.Run(async () => await ScreenShareHolder(model, sh.value, sh.i, body)));
                            }
                            await Task.WhenAll(corporateScreeningTask, Task.WhenAll(taskList));
                            apiResp = await corporateScreeningTask;
                            apiRespShareHolderList = taskList.Select(t => t.Result).ToList();
                        }
                    }
                }
                catch (Exception ex)
