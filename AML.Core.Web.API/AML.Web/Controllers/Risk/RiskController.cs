...
namespace AML.Web.Controllers.Risk
{
    ...
    public class RiskController : Controller
    {
        ...
        public ActionResult Create(RiskModel model)
        {
            ...
            try
            {
                // Up-front input checks to give user actionable error messages
                if (string.IsNullOrWhiteSpace(model.CustomerCode))
                {
                    _logger.Warn("Risk Create: CustomerCode is empty (CustomerId={0}). Aborting.", model.CustomerId);
                    _toastNotification.AddErrorToastMessage("Please select a customer before submitting.");
                }
                else if (model.RiskTypeCategoryDTO == null || model.RiskTypeCategoryDTO.Count == 0)
                {
                    _logger.Warn("Risk Create: RiskTypeCategoryDTO is null/empty for customer {0}.", model.CustomerCode);
                    _toastNotification.AddErrorToastMessage("Risk categories did not load. Reload the page and try again.");
                }
                else
                {
                    ...
                    if (result.Status == 200)
                    {
                        ...
                        _logger.Info("Risk Assessment created for customer {0}, ClientId={1}, ResultId={2}",
                            model.CustomerCode, model.ClientId, result.Result);
                    ...
                    {
                        // Repository returned non-success - log the captured message and show it
                        _logger.Error("Risk Service returned status={0} for customer {1}: {2}",
                            result.Status, model.CustomerCode, result.Message);
                        _toastNotification.AddErrorToastMessage(
                            $"Risk Assessment Creation Failed: {result.Message ?? "Unknown error"}");
                    }
                }
            }
            catch (Exception ex)
            {
                _logger.Error(ex, "Unhandled exception in Risk.Create for customer {0}", model?.CustomerCode);
                _toastNotification.AddErrorToastMessage($"Risk Assessment Creation Failed: {ex.Message}");
            }
            var clientId = _clientHandler.GetClientId();
            var lovMastersLists = _lovMasterService.GetAll();
...
.ToList();
  ...
  .ToList();
        ...
        public JsonResult GetRiskDetailsByCIdIndividual(string id, int Caseversion)
        {
            Console.WriteLine($"RiskController: Fetching risk details for Individual. clientId: {clientId}");
            var riskTypeCategories = _lovMasterService.GetAllRiskConfig("I", 1, 0, 0, clientId);
            Console.WriteLine($"RiskController: Retrieved {riskTypeCategories?.Count ?? 0} risk type categories for Individual.");

            RiskModel _riskmodel = new RiskModel
            {
                RiskTypeCategoryDTO = riskTypeCategories
            };

            // The 'get_risk_individual_by_cid' SP returns lov_type_id as a Ø-delimited
            // multi-LOV string (e.g. "1572Ø104582Ø186Ø9Ø") for some legacy records, which
            // crashes Dapper's int deserialization. Catch that here so the page just falls
            // back to the empty template instead of returning a 500 to the AJAX caller.
            RiskModel model = null;
            try
            {
                model = _mapper.Map<RiskModel>(_riskService.GetRiskDetailsOfIndividualByCID(id, Caseversion).Result);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"RiskController.GetRiskDetailsByCIdIndividual: failed to fetch risk for cid='{id}'. Falling back to empty config. Error: {ex.Message}");
                model = _riskmodel; // empty risk-type-category template, no values
                return Json(model);
            }
        ...
        public JsonResult GetRiskDetailsByCIdCorporate(string id,int Caseversion)
        {
            Console.WriteLine($"RiskController: Fetching risk details for Corporate. clientId: {clientId}");
            var riskTypeCategories = _lovMasterService.GetAllRiskConfig("C", 1, 0, 0, clientId);
            Console.WriteLine($"RiskController: Retrieved {riskTypeCategories?.Count ?? 0} risk type categories for Corporate.");
            ...
            // Same Ø-delimited multi-LOV legacy data issue as the Individual variant — degrade
            // gracefully to the empty config rather than 500-ing the AJAX caller.
            RiskCorpCustomerDTO model = null;
            try
            {
                model = _mapper.Map<RiskCorpCustomerDTO>(_riskService.GetRiskDetailsOfCorporateByCID(id, Caseversion).Result);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"RiskController.GetRiskDetailsByCIdCorporate: failed to fetch risk for cid='{id}'. Falling back to empty config. Error: {ex.Message}");
                return Json(new RiskCorpCustomerModel { RiskTypeCategoryDTO = riskTypeCategories });
            }
            ...
            for (var i = 0; i < _riskCorpmodel.RiskTypeCategoryDTO.Count; i++)
            {
                ...
                {
                    var items = _riskCorpmodel.RiskTypeCategoryDTO[i].RiskTypes[j].RiskItems;
                    if (model == null || model.ReportDataDTO == null) { continue; }
                    for (var m = 0; m < model.ReportDataDTO.Count; m++)
                    {
                        var isCountry = _riskCorpmodel.RiskTypeCategoryDTO[i].RiskTypes[j].lov_country_duplicate == 1;
                        var isNotCountryAndIdMatches = !isCountry && _riskCorpmodel.RiskTypeCategoryDTO[i].Id == model.ReportDataDTO[m].lov_type_category_id;
            ...
            if (model == null)
            {
                return Json(new RiskCorpCustomerModel { RiskTypeCategoryDTO = _riskCorpmodel.RiskTypeCategoryDTO });
            }
