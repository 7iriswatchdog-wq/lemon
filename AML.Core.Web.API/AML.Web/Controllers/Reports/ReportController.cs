...
namespace AML.Web.Controllers.Reports
{
    ...
    public class ReportController : Controller
    {
        ...
        private readonly IStmService _stmService;
        ...
        IStmService stmService)
        {
            ...
            _stmService = stmService;
        }

        /// <summary>
        /// Populate STM (Sectoral Transaction Monitoring) data on a CaseProcessModel
        /// for the customer being viewed. Behaviour:
        ///   * If menu_master.Menu_Id = 12 ("Transaction Monitoring") is NOT enabled
        ///     in client_right_master for this client - returns without touching
        ///     the model, so the view falls back to its previous behaviour.
        ///   * If enabled - sets IsStmEnabled = true, the list of sector codes
        ///     the client can see, and the customer's STM transactions (filtered
        ///     by those sectors). May return an empty transaction list - the view
        ///     should render "No transactions for this customer" in that case.
        /// </summary>
        private void AttachStmTransactionsForReport(CaseProcessModel model, string customerId, int clientId)
        {
            try
            {
                var moduleResp = _stmService.IsStmModuleEnabled(clientId);
                if (moduleResp == null || moduleResp.Status != 200 || !moduleResp.Result) return;
                model.IsStmEnabled = true;

                var sectorsResp = _stmService.GetAllowedSectorCodes(clientId);
                model.StmAllowedSectors = sectorsResp?.Result ?? new List<string>();

                if (string.IsNullOrWhiteSpace(customerId)) return;
                var txnsResp = _stmService.GetCustomerTransactions(customerId, clientId);
                model.StmTransactions = txnsResp?.Result ?? new List<StmTransactionDTO>();

                // Pull each transaction's full detail (parties) + saved risk result so the
                // PDF can render the complete picture without extra round-trips per row.
                foreach (var t in model.StmTransactions)
                {
                    try
                    {
                        var detailResp = _stmService.GetTransaction(t.Id);
                        if (detailResp?.Status == 200 && detailResp.Result != null)
                            t.Parties = detailResp.Result.Parties ?? new List<StmTransactionPartyDTO>();
                        var riskResp = _stmService.GetSavedRiskResult(t.Id);
                        if (riskResp?.Status == 200 && riskResp.Result != null)
                            model.StmTxnRiskResults[t.Id] = riskResp.Result;
                    }
                    catch { /* per-row failures stay silent; the PDF gracefully skips */ }
                }
            }
            catch (Exception ex)
            {
                // STM is supplementary - never break the report if scoring fails.
                Console.Error.WriteLine($"AttachStmTransactionsForReport failed for {customerId}/{clientId}: {ex.Message}");
            }
...
.ToList();
    ...
    .ToList();
        ...
        public IActionResult CaseReport(int type, string schedulerTrackerId, string option, string endDate = null, int caseStatus = 10)
        {
            ...
            model.EndDate = !string.IsNullOrEmpty(endDate) ? Convert.ToDateTime(endDate) : System.DateTime.Now;
    ...
    string endDate, string cust_type, string searchValue, int createdBy, string matchScore, int caseStatus, string caseStatusChange, string riskLevel, string includingDuplicate, string option)
        {
            ...
            else
            {
                if (searchValue != "" && searchValue != null)
                {
                    abc = _mapper.Map<List<CaseReportListModel>>(_reportService.GetCaseReportListBySearch(new CaseReportRequestDTO
                    {
                        ...
                        riskLevel = riskLevel,
                        includingDuplicate= includingDuplicate
    ...
    : "updatedOn";
        ...
        public async Task<IActionResult> ExportCaseManagementReport(string startDate, string endDate, string cust_type, string searchValue, int createdBy, string matchScore, int caseStatus, string caseStatusChange, string riskLevel, string includingDuplicate, bool isPDF)
        {
            var BranchId = _clientHandler.GetBranchId();
            var GroupId = _clientHandler.GetGroupId();
            var clientId = _clientHandler.GetClientId();
        ...
        public async Task<IActionResult> ExportCompletedCasesReport(string userID, string updatedByUserID, string startDate, string endDate, string status, string cust_type, string searchValue, int createdBy, string matchScore, int caseStatus, string riskLevel, string includingDuplicate, bool isPDF)
        {
            var BranchId = _clientHandler.GetBranchId();
            var GroupId = _clientHandler.GetGroupId();
            var clientId = _clientHandler.GetClientId();
            ...
            string filter = "None";
            List<string> filterList = new List<string>();
            if (!string.IsNullOrEmpty(cust_type) && cust_type != "0") filterList.Add("Customer Type: " + cust_type);
            if (caseStatus != 10) filterList.Add("Status: " + Enum.GetName(typeof(ReportsCaseStatus), caseStatus));
            if (!string.IsNullOrEmpty(riskLevel)) filterList.Add("Risk: " + riskLevel);
            if (!string.IsNullOrEmpty(matchScore)) filterList.Add("Score: " + matchScore);
            if (filterList.Count > 0) filter = string.Join(", ", filterList);
        ...
        public ActionResult ViewIndividualCaseDetail(string id,int version,int type)
        {
            CaseProcessModel model = new CaseProcessModel();
            model.Case = new CaseModel();
            int Id = _customerCaseService.GetCaseIdByVersion(id,version);//returns id of the customercase table instead of caseid
            CustomerCaseDTO _CustomerCaseDTO = null;
            if (Id > 0)
            {
                _CustomerCaseDTO = _customerCaseService.GetDetailsByVersion(Id,version);
            }
            if (_CustomerCaseDTO == null)
            {
                Id = _customerCaseService.GetCaseId(id);
                if (Id > 0)
                {
                    _CustomerCaseDTO = _customerCaseService.GetDetails(Id);
                }
            }
            model.Case = _mapper.Map<CaseModel>(_CustomerCaseDTO);

            if (model.Case == null || _CustomerCaseDTO == null)
            {
                ErrorLogDTO error = new ErrorLogDTO();
                error.created_on = DateTime.Now;
                error.createdBy = _clientHandler.GetUserId();
            ...
            // Attach STM transactions if the client has the Transaction Monitoring
            // module right enabled. No-op otherwise (preserves legacy behaviour).
            AttachStmTransactionsForReport(model, _CustomerCaseDTO?.CustomerId, _clientHandler.GetClientId());
        ...
        public ActionResult ViewIndividualCaseDetail_PDF(string id, int type)
        {
            ...
            var clientId = _clientHandler.GetClientId();
            RiskModel _riskmodel = new RiskModel();
            // Mirror the ViewIndividualCaseDetail (HTML) action verbatim so the PDF and
            // the on-screen detail page render identical risk content. Earlier attempts
            // to use the versioned GetAllRiskConfigReport here produced empty dropdowns
            // because the saved report data references LOV ids that the older
            // versioned config no longer carries.
            int riskid = _riskService.GetRiskIdByCustomercode(model.Case.CustomerId, model.Case.CustomerType).Result;
            ...
            // Same STM injection for the PDF render path.
            AttachStmTransactionsForReport(model, _CustomerCaseDTO?.CustomerId, _clientHandler.GetClientId());

            return View(model);
        }

        public ActionResult ViewShareholderCaseDetails(string id, int version,int type)
        {
            CaseProcessModel model = new CaseProcessModel();
            model.Case = new CaseModel();
            int Id = _customerCaseService.GetCaseIdByVersion(id, version);//returns id of the customercase table instead of caseid
    ...
    int orderColumn = 0, string orderDirection = "asc") // Added sorting parameters
        ...
        public async Task<IActionResult> CaseReport_PDF(string startDate, string endDate, string cust_type, string searchValue, int createdBy, string matchScore, int caseStatus, string caseStatusChange, string riskLevel, string selectedColumns, string orientation)
        {
            ...
            CaseReportRequestDTO request = new CaseReportRequestDTO
            {
                ...
                matchscore = matchScore,
                riskLevel = riskLevel == "1" ? "Low Risk" : riskLevel == "2" ? "Medium Risk" : riskLevel == "3" ? "High Risk" : riskLevel == "4" ? "Unclassified" : null
            };
        ...
        public async Task<IActionResult> DatasetUpdateLogs_PDF(string startDate, string endDate, string datasets, string searchValue = null, string selectedColumns = null, string orientation = null)
        {
            ...
            if (clientId > 0 && clientDetails != null && clientDetails.ApplicationStartDate.HasValue)
            {
                logs = logs.Where(log => log.UpdatedDate >= clientDetails.ApplicationStartDate.Value).ToList();
            }
