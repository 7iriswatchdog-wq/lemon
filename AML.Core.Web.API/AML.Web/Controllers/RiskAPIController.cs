    ...
    public class RiskAPIController : ControllerBase
    {
...

        {
            ...
            if (model.RiskCategory == "I")
            {
                ...
                riskmodel.CaseVersion = model.CaseVersion == 0 ? 1 : model.CaseVersion;
                var result = _riskService.Create(_mapper.Map<RiskDTO>(riskmodel));
                if (!string.IsNullOrWhiteSpace(model.SelectedVersions))
                {
                    var updateDto = new RiskVersionUpdateDTO
                    {
                        ...
                        RiskOverRide = riskmodel.RiskOverRide,
                        customerType= model.RiskCategory
                    };
                    var versions = model.SelectedVersions
                        .Split(',')
                        .Select(x => Convert.ToInt32(x.Trim()))
                        .ToList();

                    //var riskDto = _mapper.Map<RiskDTO>(riskmodel);

                    foreach (var version in versions)
                    {
                        _riskService.UpdateRiskAssessmentVersion(
                            updateDto,
                            version
                        );
                    }
            ...
            if (model.RiskCategory == "C")
            {
                ...
                if (!string.IsNullOrWhiteSpace(model.SelectedVersions))
                {
                    var updateDto = new RiskVersionUpdateDTO
                    {
                        CustomerCode = riskmodel.UniqueID,
                        RiskTypeCategoryDTO = riskmodel.RiskTypeCategoryDTO,
                        RiskScoreSum = riskmodel.RiskScoreSum,
                        RiskScoreCount = riskmodel.RiskScoreCount,
                        FinalRiskScore = riskmodel.RiskAssessmentRating,
                        RiskScoreBeforeOverride = riskmodel.RiskAssessmentRatingWithoutOverride,
                        RiskOverRide = riskmodel.RiskOverRide
                    };
