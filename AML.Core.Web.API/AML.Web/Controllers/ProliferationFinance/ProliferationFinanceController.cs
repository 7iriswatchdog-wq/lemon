    ...
    public class ProliferationFinanceController : Controller
    {
            ...
            try
            {
                ...
                if (response.Status == 200)
                {
                    ...
                    if (!string.IsNullOrEmpty(customerType) && customerType != "0")
                    {
                        ...
                        {
                            data = data.FindAll(x => x.CustomerType == "Military" || x.CustomerType == "Both");
                        }
                    }
        ...
        public IActionResult SearchChemicalsOnly([FromBody] ProliferationFinanceModel model)
        {
            try
            {
                ...
                else if (model.CustomerType == "Military")
                {
                    var pdfResponse = _proliferationFinanceService.SearchMilitaryGoods(model.SearchKeyword ?? model.ChemicalName);
        ...
        public IActionResult SearchChemicals([FromBody] ProliferationFinanceModel model)
        {
            try
            {
                ...
                var searchDto = new ProliferationFinanceCaseDTO
                {
                    CustomerType = string.IsNullOrWhiteSpace(model.CustomerType) ? "Goods" : model.CustomerType,
                ...
                // 2. Database Search (only if customer type is Goods, Chemical, or Both)
                if (searchDto.CustomerType == "Goods" || searchDto.CustomerType == "Chemical" || searchDto.CustomerType == "Both")
                {
                ...
                // 3. PDF Scan (respecting requested CustomerType)
                var searchKeywords = new List<string>();
                if (!string.IsNullOrWhiteSpace(model.HsCode)) searchKeywords.Add(model.HsCode.Trim());
                if (!string.IsNullOrWhiteSpace(model.CasNumber)) searchKeywords.Add(model.CasNumber.Trim());
                if (!string.IsNullOrWhiteSpace(model.Eccn)) searchKeywords.Add(model.Eccn.Trim());
                if (!string.IsNullOrWhiteSpace(model.ChemicalName)) searchKeywords.Add(model.ChemicalName.Trim());
                if (!string.IsNullOrWhiteSpace(model.SynonymName)) searchKeywords.Add(model.SynonymName.Trim());

                var nonChemHits = new List<string>();
                var militaryHits = new List<string>();

                foreach (var kw in searchKeywords.Distinct(StringComparer.OrdinalIgnoreCase))
                {
                    if (searchDto.CustomerType == "Goods" || searchDto.CustomerType == "Non-Chemical" || searchDto.CustomerType == "Both")
                    {
                        var nonChemRes = _proliferationFinanceService.SearchNonChemical(kw);
                        if (nonChemRes.Status == 200 && !string.IsNullOrEmpty(nonChemRes.Result))
                        {
                            nonChemHits.Add(nonChemRes.Result);
                        }
                    }

                    if (searchDto.CustomerType == "Goods" || searchDto.CustomerType == "Military" || searchDto.CustomerType == "Both")
                    {
                        var milRes = _proliferationFinanceService.SearchMilitaryGoods(kw);
                        if (milRes.Status == 200 && !string.IsNullOrEmpty(milRes.Result))
                        {
                            militaryHits.Add(milRes.Result);
                        }
                    }
                }

                if (nonChemHits.Count > 0 || militaryHits.Count > 0)
                {
                    pdfMatch = true;
                    var combinedHits = nonChemHits.Concat(militaryHits).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
                    searchDto.SearchHitDetails = string.Join("\n\n", combinedHits);
                    matchSources.Add("Document Scan");

                    // Set CustomerType specifically based on where the hits came from and what was requested
                    if (searchDto.CustomerType == "Goods")
                    {
                        if (militaryHits.Count > 0 && nonChemHits.Count > 0)
                        {
                            searchDto.CustomerType = "Both";
                        }
                        else if (militaryHits.Count > 0)
                        {
                            searchDto.CustomerType = "Military";
                        }
                        else if (nonChemHits.Count > 0)
                        {
                            searchDto.CustomerType = "Non-Chemical";
                        }
                    }
                }
                ...
                // 5. Update Case Metadata based on results
                searchDto.Score = "100";
                searchDto.StatusReason = "";
                ...
                if (createResponse.Status == 200 && createResponse.Result > 0)
                {
                    ...
                    remarkModel.Comment = "PF Case is Created";
                    ...
                    try
                    {
                        ...
                        // Sync Chemical matches
                        _proliferationFinanceService.SyncSearchResultsToMongo(caseId, model.ChemicalName ?? "", chemMatches, null, "Chemical");
                        
                        // Sync Non-Chemical matches if any
                        if (nonChemHits != null && nonChemHits.Count > 0)
                        {
                            _proliferationFinanceService.SyncSearchResultsToMongo(caseId, model.ChemicalName ?? "", null, string.Join("\n\n", nonChemHits), "Non-Chemical");
                        }
                        
                        // Sync Military Goods matches if any
                        if (militaryHits != null && militaryHits.Count > 0)
                        {
                            _proliferationFinanceService.SyncSearchResultsToMongo(caseId, model.ChemicalName ?? "", null, string.Join("\n\n", militaryHits), "Military");
                        }
                    }
        ...
        public IActionResult Process(int id)
        {
            try
            {
                ...
                // For Chemical/Goods/Military/Both cases, re-fetch ALL potential hits from UAE Control List
                if (model.CustomerType == "Goods" || model.CustomerType == "Chemical" || model.CustomerType == "Military" || model.CustomerType == "Both")
                {
                ...
                // Deduplicate PDF snippets and classify them
                var nonChemSnippets = new List<string>();
                var militarySnippets = new List<string>();
                if (!string.IsNullOrEmpty(model.SearchHitDetails))
                {
                    ...
                    string nonChemPath = Path.Combine(Directory.GetCurrentDirectory(), "Files", "Non-chemical.pdf");
                    string militaryPath = Path.Combine(Directory.GetCurrentDirectory(), "Files", "Stand-alone-Munitions-List-2025-Corr.pdf");

                    foreach (var s in uniqueSnippets)
                    {
                        if (AML.Core.Common.CommonClasses.PdfHelper.IsParagraphInPdf(militaryPath, s))
                        {
                            militarySnippets.Add(s);
                        }
                        else
                        {
                            nonChemSnippets.Add(s);
                        }
                    }

                    model.SearchHitDetails = string.Join("\n\n", uniqueSnippets);
                    ViewBag.NonChemCount = nonChemSnippets.Count;
                    ViewBag.MilitaryCount = militarySnippets.Count;
                }
                else
                {
                    ViewBag.NonChemCount = 0;
                    ViewBag.MilitaryCount = 0;
                }

                // 1. Sync fresh results to MongoDB (Detection of new matches)
                _proliferationFinanceService.SyncSearchResultsToMongo(id, model.ChemicalName, model.MatchedChemicals, null, "Chemical");
                if (nonChemSnippets.Count > 0)
                {
                    _proliferationFinanceService.SyncSearchResultsToMongo(id, model.ChemicalName, null, string.Join("\n\n", nonChemSnippets), "Non-Chemical");
                }
                if (militarySnippets.Count > 0)
                {
                    _proliferationFinanceService.SyncSearchResultsToMongo(id, model.ChemicalName, null, string.Join("\n\n", militarySnippets), "Military");
                }
        ...
        public IActionResult Details(int id, int? chemicalId = null)
        {
            try
            {
                ...
                string typeParam = Request.Query["type"].ToString().ToLower();
                if (!string.IsNullOrEmpty(model.SearchHitDetails) && !string.IsNullOrEmpty(typeParam))
                {
                    var snippets = model.SearchHitDetails.Split(new[] { "\n\n" }, StringSplitOptions.RemoveEmptyEntries);
                    var filteredSnippets = new List<string>();
                    string militaryPath = Path.Combine(Directory.GetCurrentDirectory(), "Files", "Stand-alone-Munitions-List-2025-Corr.pdf");

                    foreach (var s in snippets)
                    {
                        bool isMilitary = AML.Core.Common.CommonClasses.PdfHelper.IsParagraphInPdf(militaryPath, s);
                        if (typeParam == "military" && isMilitary)
                        {
                            filteredSnippets.Add(s);
                        }
                        else if (typeParam == "non-chemical" && !isMilitary)
                        {
                            filteredSnippets.Add(s);
                        }
                    }
                ...
                else if (model.CustomerType == "Goods" || model.CustomerType == "Chemical" || model.CustomerType == "Military" || model.CustomerType == "Both")
                {
        ...
        [HttpGet("/ProliferationFinance/DownloadDetails/{id}")]
        public async Task<IActionResult> DownloadDetails(int id, string type = null)
        {
            try
            {
                var caseDetails = _proliferationFinanceService.GetCaseById(id);
                ...
                if (!string.IsNullOrEmpty(model.SearchHitDetails) && !string.IsNullOrEmpty(type))
                {
                    var typeParam = type.ToLower();
                    var snippets = model.SearchHitDetails.Split(new[] { "\n\n" }, StringSplitOptions.RemoveEmptyEntries);
                    var filteredSnippets = new List<string>();
                    string militaryPath = Path.Combine(Directory.GetCurrentDirectory(), "Files", "Stand-alone-Munitions-List-2025-Corr.pdf");
                        ...
                        for (int row = 2; row <= rowCount; row++)
                        {
                            try
                            {
                                ...
                                if (hasChemicalFields || hasNonChemicalFields)
                                {
                                    ...
                                    // 2. Document Scan (PDF Scan for both PDFs)
                                    var searchKeywords = new List<string>();
                                    if (!string.IsNullOrWhiteSpace(hsCode)) searchKeywords.Add(hsCode.Trim());
                                    if (!string.IsNullOrWhiteSpace(casNumber)) searchKeywords.Add(casNumber.Trim());
                                    if (!string.IsNullOrWhiteSpace(eccn)) searchKeywords.Add(eccn.Trim());
                                    if (!string.IsNullOrWhiteSpace(chemicalName)) searchKeywords.Add(chemicalName.Trim());
                                    if (!string.IsNullOrWhiteSpace(synonymName)) searchKeywords.Add(synonymName.Trim());
                                    if (!string.IsNullOrWhiteSpace(searchKeyword)) searchKeywords.Add(searchKeyword.Trim());
                                    ...
                                    if (nonChemHits.Count > 0 || militaryHits.Count > 0)
                                    {
                                        pdfMatch = true;
                                        var combinedHits = nonChemHits.Concat(militaryHits).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
                                        caseDto.SearchHitDetails = string.Join("\n\n", combinedHits);
                                        matchSources.Add("Document Scan");

                                        if (militaryHits.Count > 0 && nonChemHits.Count > 0)
                                        {
                                            caseDto.CustomerType = "Both";
                                        }
                                        else if (militaryHits.Count > 0)
                                        {
                                            caseDto.CustomerType = "Military";
                                        }
                                        else if (nonChemHits.Count > 0)
                                        {
                                            caseDto.CustomerType = "Non-Chemical";
                                        }
                                        
                                        if (string.IsNullOrEmpty(caseDto.ChemicalName))
                                        {
                                            caseDto.ChemicalName = searchKeywords.FirstOrDefault() ?? "";
                                        }
                                    }
        ...
        public IActionResult SaveSearchHits([FromBody] SearchHitsRequest request)
        {
            try
            {
                ...
                {
                    if (!string.IsNullOrEmpty(hit.Decision) || !string.IsNullOrEmpty(hit.Remarks))
                    {
                        _proliferationFinanceService.UpdateMongoHitDecision(request.CaseId, hit.SearchType, hit.Decision, hit.Remarks);
                        ...
                        {
                            if (_CustomerCaseDTO.Type == "Corporate")
                            {
                                ...
                                if (riskid != 0)
                                {
                                    ...
                                    modelrisk = _mapper.Map<RiskCorpCustomerModel>(_riskService.GetRiskDetailsOfCorporateByCID(request.corporateId, _CustomerCaseDTO.Version).Result);

                                    if (modelrisk != null && modelrisk.ReportDataDTO != null)
                                    {
                                        ...
                                        foreach (var riskType in category.RiskTypes)
                                        {
                                            if (!string.IsNullOrEmpty(riskType.ItemTxt))
                                            {
                                                ...
                                                case "dual use goods match":
                                                    ...
                                                    break;
                                                case "military goods match":
                                                    corpModel.MilitaryGoods = riskType.ItemTxt;
                                                    break;
                                                case "if more military goods":
                                                    corpModel.MoreMilitaryGoods = riskType.ItemTxt;
                                                    break;
                            ...
                            if (hit.SearchType == "Military")
                            {
                                if (!string.IsNullOrEmpty(hit.Decision))
                                {
                                    if (_CustomerCaseDTO.Version > 1)
                                    {
                                        corpModel.MoreMilitaryGoods = (hit.Decision == "True Match" || hit.Decision == "Potential Match") ? "Yes" : "No";
                                    }
                                    else
                                    {
                                        corpModel.MilitaryGoods = (hit.Decision == "True Match" || hit.Decision == "Potential Match") ? "Yes" : "No";
                                    }
                                }
                            }
                            else if (hit.SearchType == "Chemical" || hit.SearchType == "Non-Chemical" || hit.SearchType == "Goods")
                            {
                                if (!string.IsNullOrEmpty(hit.Decision))
                                {
                                    if (_CustomerCaseDTO.Version > 1)
                                    {
                                        corpModel.MoreDualUseGoods = (hit.Decision == "True Match" || hit.Decision == "Potential Match") ? "Yes" : "No";
                                    }
                                    else
                                    {
                                        corpModel.DualUseGoods = (hit.Decision == "True Match" || hit.Decision == "Potential Match") ? "Yes" : "No";
                                    }
                                }
                            }
                            ...
                            if (militarygoodsId != "0")
                            {
                                var riskType22 = new RiskTypeListModel();
                                riskType22.Id = Convert.ToString(militarygoodslovId);
                                var riskItem22 = new RiskItemListModel();
                                riskItem22.Id = militarygoodsId.ToString();
                                var riskItemList22 = new List<RiskItemListModel>();
                                riskItemList22.Add(riskItem22);
                                riskType22.RiskItemList = riskItemList22;
                                riskTypeList.Add(riskType22);
                            }
                            if (MoremilitarygoodsId != "0")
                            {
                                var riskType23 = new RiskTypeListModel();
                                riskType23.Id = Convert.ToString(MoremilitarygoodslovId);
                                var riskItem23 = new RiskItemListModel();
                                riskItem23.Id = MoremilitarygoodsId.ToString();
                                var riskItemList23 = new List<RiskItemListModel>();
                                riskItemList23.Add(riskItem23);
                                riskType23.RiskItemList = riskItemList23;
                                riskTypeList.Add(riskType23);
                            }
                        ...
                        else
                        {
                            ...
                            if (riskid != 0)
                            {
                                ...
                                modelrisk = _mapper.Map<RiskModel>(_riskService.GetRiskDetailsOfIndividualByCID(request.corporateId, _CustomerCaseDTO.Version).Result);
                                        ...
                                        if (!string.IsNullOrEmpty(riskType.ItemTxt))
                                        {
                                            switch (riskType.RiskType?.ToLower()) // or riskType.Code (better if available)
                                            {
                                                ...
                                                case "dual use goods match":
                                                    ...
                                                    break;
                                                case "military goods match":
                                                    imodel.MilitaryGoods = riskType.ItemTxt;
                                                    break;
                                                case "if more military goods":
                                                    imodel.MoreMilitaryGoods = riskType.ItemTxt;
                                                    break;
                                            }
                                        }
                                    }
                                ...
                                if (hit.SearchType == "Military")
                                {
                                    if (!string.IsNullOrEmpty(hit.Decision))
                                    {
                                        if (_CustomerCaseDTO.Version > 1)
                                        {
                                            imodel.MoreMilitaryGoods = (hit.Decision == "True Match" || hit.Decision == "Potential Match") ? "Yes" : "No";
                                        }
                                        else
                                        {
                                            imodel.MilitaryGoods = (hit.Decision == "True Match" || hit.Decision == "Potential Match") ? "Yes" : "No";
                                        }
                                    }
                                }
                                else if (hit.SearchType == "Chemical" || hit.SearchType == "Non-Chemical" || hit.SearchType == "Goods")
                                {
                                    if (!string.IsNullOrEmpty(hit.Decision))
                                    {
                                        if (_CustomerCaseDTO.Version > 1)
                                        {
                                            imodel.MoreDualUseGoods = (hit.Decision == "True Match" || hit.Decision == "Potential Match") ? "Yes" : "No";
                                        }
                                        else
                                        {
                                            imodel.DualUseGoods = (hit.Decision == "True Match" || hit.Decision == "Potential Match") ? "Yes" : "No";
                                        }
                                    }
                                }
                                ...
                                {
                                    var riskType15 = new RiskTypeListModel();
                                    riskType15.Id = Convert.ToString(militarygoodslovId);
                                    var riskItem15 = new RiskItemListModel();
                                    riskItem15.Id = militarygoodsId.ToString();
                                    var riskItemList15 = new List<RiskItemListModel>();
                                    riskItemList15.Add(riskItem15);
                                    riskType15.RiskItemList = riskItemList15;
                                    riskTypeList.Add(riskType15);
                                ...
                                {
                                    var riskType16 = new RiskTypeListModel();
                                    riskType16.Id = Convert.ToString(MoremilitarygoodslovId);
                                    var riskItem16 = new RiskItemListModel();
                                    riskItem16.Id = MoremilitarygoodsId.ToString();
                                    var riskItemList16 = new List<RiskItemListModel>();
                                    riskItemList16.Add(riskItem16);
                                    riskType16.RiskItemList = riskItemList16;
                                    riskTypeList.Add(riskType16);
                ...
                if (!string.IsNullOrEmpty(result1))
                {
                    try 
                    {
                        var token = Newtonsoft.Json.Linq.JToken.Parse(result1);
                        if (token is Newtonsoft.Json.Linq.JArray)
                        {
                            jsonList = token.ToObject<List<DataListModel>>() ?? new List<DataListModel>();
                        }
                        else
                        {
                            var responseModel = token.ToObject<CaseLogResponseModel>();
                            jsonList = responseModel?.MatchRecords ?? new List<DataListModel>();
                        }
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine("ProliferationFinanceController SaveSearchHits deserialization error: " + ex.Message);
                    }
                ...
                if (request.Hits != null && request.Hits.Any(h => !string.IsNullOrEmpty(h.Decision)))
                {
                    foreach (var hit in request.Hits.Where(h => !string.IsNullOrEmpty(h.Decision)))
                    {
                        comments.Add(($"{hit.SearchType} Action: {hit.Decision} Found And Saved", "Proliferation Finance"));
                    }
                }
                else
                {
                    comments.Add(("Details Found And Saved", "Proliferation Finance"));
                }
                ...
                foreach (var item in comments)
                {
                    ...
                    {
                        CaseId = Id,
                        CustomerId= request.corporateId,
