    ...
    return {
        ...
        mapNodeToDto(node) {
            ...
            const dto = {
                ...
                MatchThreshold: parseInt(node.matchThreshold || node.threshold) || 70,
                ...
                GroupId: Array.isArray(node.groupId) ? node.groupId.join(',') : (node.groupId || ''),
                GroupRisk: Array.isArray(node.groupRisk) ? node.groupRisk.join(',') : (node.groupRisk || '')
            };
                    ...
                    this.savedDraftPayload.nodes.forEach((nodeDto, index) => {
                        ...
                        if (!nodesRaw.get(nodeId)) {
                            ...
                            nodesRaw.add({
                                ...
                                image: this.getSvg(type, isRootNode, !!(nodeDto.isScreened || nodeDto.IsScreened), !!(nodeDto.isDuplicate || nodeDto.IsDuplicate), caseId),
                                type: type,
                                isRoot: isRootNode,
        ...
        init() {
            ...
            network.on('click', (params) => {
                ...
                if (this.activeTool === 'connect' && params.nodes.length === 1) {
                    ...
                    this.activeEdgeRelationship = edgeObj ? (edgeObj.relationship || 'individual_shareholder') : 'individual_shareholder';
        ...
        autoLayout(type = 'adaptive', silent = false) {
            ...
            if (type === 'hierarchical') {
                ...
                network.setOptions({
                    ...
                    edges: {
                        smooth: { type: 'horizontal', roundness: 0.5 }
                    },
                    physics: {
                        enabled: true,
        ...
        updateHierarchyTypes(nodeId) {
            ...
            const parentId = incomingEdges.length > 0 ? incomingEdges[0].from : null;
            ...
            nodes.update({ id: nodeId, shareholderType: newType, flagType: flagType });
        ...
        getTreeNodes() {
            ...
            const topLevelNodes = nodes.filter(n => n.isRoot || !edges.some(e => e.to === n.id))
        ...
        dropTree(e, targetId) {
            ...
            // Reparent: 
            // 1. Remove ALL existing incoming edges to sourceId (it can only have one parent)
        ...
        dropOnRoot(e) {
            ...
            const nodesRaw = Alpine.raw(this.nodes);
            nodesRaw.update({ id: sourceId, isRoot: true });
            ...
            try {
                ...
                if (entities && entities.length > 0) {
                    ...
                    entities.forEach((entity, index) => {
                        ...
                        if (!nodes.get(nodeId)) {
                            ...
                            nodes.add({
                                ...
                                x: viewCenter.x + (index * 50),
                                y: viewCenter.y + (index * 50),
                                image: this.getSvg(type, isRootNode, true, !!(entity.isDuplicate || entity.IsDuplicate), caseId),
                                type: type,
                                isRoot: isRootNode,
                                isScreened: true,
                    ...
                    entities.forEach(entity => {
                        ...
                        if (parentId && parentId.toString() !== "0" && parentId.toString() !== sNodeId && nodes.get(parentId.toString()) && nodes.get(sNodeId)) {
                            ...
                            if (!edges.get(edgeId)) {
                                edges.add({
                                    ...
                                    to: sNodeId,
                                    label: entity.relationship || entity.Relationship || ''
                                });
                            }
                        }
                    });
            ...
            try {
                ...
                if (result && result.entity) {
                    ...
                    const newNode = {
                        ...
                        image: this.getSvg(type, isRoot, true, true),
                        type: type,
                        isRoot: isRoot,
                        ...
                        level: orphanNode ? orphanNode.level : 0
                    };
        ...
        addNode(type, isRoot, x, y, roleId = null) {
            ...
            nodes.add({
                id, label, x, y,
                image: this.getSvg(type, isRoot, false, false),
        ...
        promoteNode(nodeId) {
            ...
            if (currentRoot) {
                nodes.update({
                    ...
                    isRoot: false,
                    level: 1,
                    image: this.getSvg(currentRoot.type || (currentRoot.customerType === 'C' ? 'C' : 'I'), false, currentRoot.isScreened, currentRoot.isDuplicate, currentRoot.caseId)
                });
            }
            ...
            nodes.update({
                ...
                isRoot: true,
                level: 0,
                image: this.getSvg(targetNode.type || (targetNode.customerType === 'C' ? 'C' : 'I'), true, targetNode.isScreened, targetNode.isDuplicate, targetNode.caseId)
            });
        ...
        openNode(node) {
            ...
            base.isRoot = !!node.isRoot || data.isRoot || data.IsRoot || node.relationship === 'root' || data.relationship === 'root' || data.FlagType === 'root';
            ...
            let name = data.name || data.Name || data.firstName || data.FirstName || data.lastName || data.LastName || data.companyName || data.CompanyName || data.fullName || data.FullName || data.onb_name || '';
            ...
            base.relationship = data.relationship || data.Relationship || data.flagType || data.FlagType || (node.parentID ? 'child' : 'root');
            ...
            // Meta & Scoring
            base.matchParameter = data.matchParameter || data.threshold || data.Threshold || 80;
        ...
        updateNode() {
            ...
            const updated = {
                ...
                image: this.getSvg(this.formData.type, this.formData.isRoot, this.formData.isScreened, this.formData.isDuplicate)
            };
            this.nodes.update({
                id: this.selectedNode.id,
                label: updated.label,
        ...
        quickAdd(type, parentId) {
            ...
            nodes.add({
                ...
                image: this.getSvg(type, false, false, false),
        ...
        deleteHoveredNode() {
            ...
            if (this.hoveredNodeId) {
                this.nodes.remove(this.hoveredNodeId);
                this.hoveredNodeId = null;
                this.updateBoundingBox();
                this.updateTree();
            }
                ...
                if (targetNode) {
                    ...
                    if (role) {
                        ...
                        nodes.update({
                            ...
                            relationship: val,
                            shareholderType: shType,
                            image: this.getSvg(role.type, targetNode.isRoot, targetNode.isScreened, targetNode.isDuplicate)
                        });
                        this.updateHierarchyTypes(edge.to);
                    }
                }
            ...
            try {
                ...
                if (result && result.entity) {
                    ...
                    nodes.add({
                        ...
                        level: 0,
                        label: finalName,
                        image: this.getSvg(type, false, true, false, caseId),
                        type: type,
                        isRoot: false,
                        isScreened: true,
                        isFetched: true
        ...
        getSvg(type, isRoot, isScreened, isDuplicate) {
            const color = isRoot ? '#2563EB' : '#64748B';
            const bg = isRoot ? '#EFF6FF' : '#F8FAFC';
            ...
            const icons = {
                ...
                C_child: '<rect width="16" height="20" x="4" y="2" rx="2" ry="2"/><path d="M9 22v-4h6v4"/><path d="M8 6h.01"/><path d="M16 6h.01"/><path d="M12 6h.01"/><path d="M12 10h.01"/><path d="M12 14h.01"/><path d="M16 10h.01"/><path d="M16 14h.01"/><path d="M8 10h.01"/><path d="M8 14h.01"/>',
                I_root: '<path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/>',
                I_child: '<path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><line x1="19" y1="8" x2="19" y2="14"/><line x1="16" y1="11" x2="22" y2="11"/>'
            };

            const iconPath = icons[`${type}_${isRoot ? 'root' : 'child'}`];
            ...
            const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="160" height="160" viewBox="0 0 160 160">
                <rect x="10" y="10" width="140" height="140" rx="32" fill="${bg}" stroke="${color}" stroke-width="4"/>
                <g transform="translate(40, 40) scale(3.5)" stroke="${color}" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round">${iconPath}</g>
                ${lockIcon}
                ${dupIcon}
            </svg>`;
        ...
        switchNodeType() {
            ...
            nodes.update({
                id: this.selectedNode.id,
                type: newType,
                image: this.getSvg(newType, false, this.formData.isScreened, this.selectedNode.isDuplicate)
            });
        ...
        setRelatedPartyRole(roleId) {
            ...
            if (this.selectedNode) {
                this.nodes.update({
                    ...
                    image: this.getSvg(role.type, false, this.formData.isScreened)
                });
                this.updateHierarchyTypes(this.selectedNode.id);
            }
            this.$nextTick(() => lucide.createIcons());
        ...
        createEdge(fromId, toId) {
            ...
            // Update node relationship if it's currently empty
            if (!toNode.relationship) {
                const defaultRole = toNode.type === 'C' ? 'corporate_shareholder' : 'individual_shareholder';
                const defaultSHType = toNode.type === 'C' ? 'Corporate' : 'Individual';
                nodes.update({
                    id: toId,
                    relationship: defaultRole,
                    shareholderType: defaultSHType
                });
                toNode.relationship = defaultRole;
                toNode.shareholderType = defaultSHType;
            }

            const fromNode = nodes.get(fromId);
            if (toNode && toNode.isRoot) {
                // If we are linking something TO the current root, the root becomes a child
                nodes.update({ 
                    id: toId, 
                    isRoot: false, 
                    image: this.getSvg(toNode.type, false, toNode.isScreened, toNode.isDuplicate, toNode.caseId) 
                });
                ...
                if (!hasOtherParent) {
                    nodes.update({ 
                        id: fromId, 
                        isRoot: true, 
                        image: this.getSvg(fromNode.type, true, fromNode.isScreened, fromNode.isDuplicate, fromNode.caseId) 
                    });
                    if (this.selectedNode && this.selectedNode.id === fromId) this.formData.isRoot = true;
                }
            }
        ...
        async saveCase() {
            ...
            if (res.isConfirmed) {
                ...
                // Order and expand nodes starting from top-level roots to duplicate multi-parented branches
                const topLevelNodes = rawNodes.filter(n => n.isRoot || !rawEdges.some(e => e.to === n.id))
                    .sort((a, b) => a.id.toString().localeCompare(b.id.toString()));

                const processNode = (node, parentEdge = null, parentStudioId = null, parentShType = null) => {
                    const edgeKey = parentEdge ? `${parentEdge.from}->${parentEdge.to}@${parentEdge.id || ''}` : `root->${node.id}`;
                    if (visitedEdges.has(edgeKey)) return;
                    visitedEdges.add(edgeKey);

                    const canvasId = node.id.toString();
                    nodeVisitCounts[canvasId] = (nodeVisitCounts[canvasId] || 0) + 1;
                    const count = nodeVisitCounts[canvasId];

                    let currStudioId = canvasId;
                    if (count > 1) {
                        currStudioId = `${canvasId}_parent_${parentStudioId || parentEdge?.from}`;
                    }

                    const dto = this.mapNodeToDto(node);
                    dto.StudioId = currStudioId;
                    if (count > 1) {
                        dto.Id = '0';
                        dto.IsFetched = false;
                        dto.IsRoot = false;
                    ...
                    const base = node.type === 'C' ? 'Corp' : 'Ind';
                    if (parentEdge && parentStudioId) {
                        let pType = parentShType;
                        if (!pType) {
                            const pNode = rawNodes.find(n => n.id.toString() === parentEdge.from.toString());
                            pType = pNode ? (pNode.type === 'C' ? 'Corporate' : 'Individual') : 'Corporate';
                        }
                    ...
                    } else if (node.isRoot || !parentEdge) {
                        dto.ShareholderType = node.type === 'C' ? 'Corporate' : 'Individual';
                        dto.Type = dto.ShareholderType;
                    }

                    finalNodes.push(dto);
                    addedNodeIds.add(currStudioId);

                    if (parentEdge && parentStudioId) {
                        finalEdges.push({
                            From: parentStudioId.toString(),
                            To: currStudioId.toString(),
                            Relationship: parentEdge.relationship || '',
                            Share: parentEdge.share || null,
                            Designation: parentEdge.designation || '',
                            Remarks: parentEdge.remarks || '',
                            Label: parentEdge.label || ''
                        });
                    }

                    // Process outgoing edges
                    const outgoingEdges = rawEdges.filter(e => e.from.toString() === canvasId);
                    outgoingEdges.forEach(edge => {
                        const childNode = rawNodes.find(n => n.id.toString() === edge.to.toString());
                        if (childNode) {
                            processNode(childNode, edge, currStudioId, dto.ShareholderType);
                        }
                    });
                };

                topLevelNodes.forEach(n => {
                    processNode(n, null, null, null);
                });

                rawNodes.forEach(n => {
                    const canvasId = n.id.toString();
                    if (!nodeVisitCounts[canvasId]) {
                        processNode(n, null, null, null);
                    }
                });
