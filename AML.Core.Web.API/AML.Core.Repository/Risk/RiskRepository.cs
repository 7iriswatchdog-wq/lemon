    ...
    public class RiskRepository : BaseRepository, IRiskRepository
    {
        ...
        public ServiceResponse<int> Create(RiskDTO _riskDTO)
        {
            ...
            try
            {
                ...
                parameters.Add("@p_caseVersion", _riskDTO.CaseVersion);
        ...
        public ServiceResponse<bool> UpdateRiskAssessmentVersion(RiskVersionUpdateDTO riskDTO,int assessmentVersion)
        {
            ...
            try
            {
                ...
                parameters.Add("@p_customercode", riskDTO.CustomerCode);
                parameters.Add("@p_assessment_version", assessmentVersion);
                parameters.Add("@p_risk_score_sum", riskDTO.RiskScoreSum);
                parameters.Add("@p_final_risk_score", riskDTO.FinalRiskScore);
                parameters.Add("@p_risk_score_count", riskDTO.RiskScoreCount);
                parameters.Add("@p_risk_score_before_override", riskDTO.RiskScoreBeforeOverride);
                parameters.Add("@p_riskoverride", riskDTO.RiskOverRide);

                string riskItemlist = "";
                int index2 = 0;

                for (int i = 0; i < riskDTO.RiskTypeCategoryDTO.Count; i++)
                {
                    for (int j = 0; j < riskDTO.RiskTypeCategoryDTO[i].RiskTypes.Count; j++)
                    {
                        if (riskDTO.RiskTypeCategoryDTO[i].RiskTypes[j].SelectedItemId != 0)
                        {
                            riskItemlist +=
                                riskDTO.RiskTypeCategoryDTO[i].RiskTypes[j].Id +
                                "Ø" +
                                riskDTO.RiskTypeCategoryDTO[i].RiskTypes[j].SelectedItemId +
                                "¥";
                ...
                parameters.Add("@p_risk_data", riskItemlist);
                parameters.Add("@p_risk_category_count", index2);
                parameters.Add("@p_customerType", riskDTO.customerType);
                Execute("upd_transaction_risk_assessment_version",parameters,commandType: CommandType.StoredProcedure);
                serviceResponse.Result = true;
                serviceResponse.Status = StaticResource.SuccessStatusCode;
                serviceResponse.Message = "Risk Assessment updated successfully.";
            }
            catch (Exception ex)
            {
        ...
        public ServiceResponse<RiskDTO> GetRiskDetailsOfIndividualByCID(string id,int Caseversion)
        {
            ...
            try
            {
                DynamicParameters parameters = new DynamicParameters();
                parameters.Add("@p_id", id);
                parameters.Add("@p_caseVersion", Caseversion);
