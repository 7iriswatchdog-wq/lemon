    ...
    catch (Exception)
    {
        ...
        public bool InsertCaseLog(CASELOG model)
        {
            try
            {
                ...
                var dxwebmatchRecord = new List<WEBLOGMATCH>();
                ...
                if (model.WEBRECORDS != null && model.WEBRECORDS.Count() > 0)
                {
                    foreach (var item in model.WEBRECORDS)
                    {
                        dxwebmatchRecord.Add(new WEBLOGMATCH
                        {
                            SHORTDESCRIPTION = string.IsNullOrEmpty(item.SHORTDESCRIPTION) ? "" : item.SHORTDESCRIPTION.Trim().ToUpper(),
                            URL= string.IsNullOrEmpty(item.URL) ? "" : item.URL.Trim(),
                            TITLE = string.IsNullOrEmpty(item.TITLE) ? "" : item.TITLE.Trim().ToUpper(),
                            MATCHSCORE = item.MATCHSCORE,
                            MATCHPRESENT = item.MATCHPRESENT,
                            MATCHEDNAMETERMS = item.MATCHEDNAMETERMS ?? new List<string>(),
                            CRIMEKEYWORDSFOUND = item.CRIMEKEYWORDSFOUND ?? new List<string>(),
                            ENGINE = string.IsNullOrEmpty(item.ENGINE) ? "" : item.ENGINE.Trim().ToUpper()
                        });
                    }
                }
                #endregion
