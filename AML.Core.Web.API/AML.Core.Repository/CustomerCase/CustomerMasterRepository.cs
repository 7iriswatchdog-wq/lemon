    ...
    public class CustomerMasterRepository : BaseRepository, ICustomerMasterRepository
    {
        ...
        public ServiceResponse<string> Create(CustomerMasterDTO _CustomerMasterDTO)
        {
            ...
            try
            {
                ...
                parameters.Add("@p_placeofbirth", _CustomerMasterDTO.PlaceOfBirth);
                ...
                parameters.Add("@p_is_duplicate", _CustomerMasterDTO.IsDuplicate);
                parameters.Add("@p_version", _CustomerMasterDTO.Version);
                parameters.Add("@p_group_id", _CustomerMasterDTO.GroupId);
                parameters.Add("@p_group_risk", _CustomerMasterDTO.GroupRisk);
                parameters.Add("@p_replaceflag", _CustomerMasterDTO.ReplaceFlag);
                parameters.Add("@p_placeofbirth", _CustomerMasterDTO.PlaceOfBirth);
        ...
        public ServiceResponse<string> CreatePrefix(CustomerMasterDTO _CustomerMasterDTO)
        {
            ...
            try
            {
                ...
                Console.WriteLine(string.Format("FirstName: {0}" +
                    ...
                    _CustomerMasterDTO.CustomerId,
                    _CustomerMasterDTO.customerCodeprefix,
        ...
        public ServiceResponse<int> Insert(CustomerMasterDTO _CustomerMasterDTO)
        {
            ...
            try
            {
                ...
                parameters.Add("@p_created_by", _CustomerMasterDTO.CreatedBy);
                parameters.Add("@p_clientId", _CustomerMasterDTO.ClientId);
                parameters.Add("@p_companyCode", _CustomerMasterDTO.GroupEntityof);
                parameters.Add("@p_is_duplicate", _CustomerMasterDTO.IsDuplicate);
        ...
        public ServiceResponse<int> UpdateCustomerMaster(CustomerMasterDTO _CustomerMasterDTO)
        {
            ServiceResponse<int> serviceResponse = new ServiceResponse<int>();
            try
            {
                DynamicParameters parameters = new DynamicParameters();
                parameters.Add("@p_id", _CustomerMasterDTO.CustomerMasterId);
                //parameters.Add("@p_cust_id_type", _CustomerMasterDTO.CustomerIdType);
                //parameters.Add("@p_cust_id_number", _CustomerMasterDTO.CustomerIdNumber);
                parameters.Add("@p_updated_by", _CustomerMasterDTO.UpdatedBy);
                parameters.Add("@p_updated_on", DateTime.UtcNow, DbType.DateTime);
                ...
                parameters.Add("@p_desgination", _CustomerMasterDTO.Designation);
                parameters.Add("@p_counterpartyname", _CustomerMasterDTO.CounterPartyName);
                parameters.Add("@p_updatereason", _CustomerMasterDTO.UpdateReason);
                var response = ExecuteScalar("mod_customer_master", parameters, commandType: CommandType.StoredProcedure).ParseInt();
                serviceResponse.Result = response;
                serviceResponse.Message = "Customer master updated successfully.";
                serviceResponse.Status = StaticResource.SuccessStatusCode;
            }
        ...
        public void TouchUpdatedOn(int numericId, int updatedBy)
        {
            const string sql = "UPDATE customercase SET updated_on = @updatedOn, updated_by = @updatedBy, status = 0 WHERE id = @id";
            Execute(sql, new { id = numericId, updatedOn = DateTime.Now, updatedBy }, commandType: System.Data.CommandType.Text);
        }

        public void UpdateGroupInfo(int numericId, string groupId, string groupRisk, int isDuplicate = 0, string groupEntityOf = null)
        {
            const string sql1 = @"UPDATE customermaster SET group_id = @groupId, group_risk = @groupRisk, group_entity_of = @groupEntityOf, is_duplicate = @isDuplicate 
                               WHERE cust_ref_id = (SELECT cust_id FROM customercase WHERE id = @id);";
            const string sql2 = @"UPDATE customercase SET is_duplicate = @isDuplicate 
                               WHERE id = @id;";
            Execute(sql1, new { id = numericId, groupId, groupRisk, isDuplicate, groupEntityOf }, commandType: System.Data.CommandType.Text);
            Execute(sql2, new { id = numericId, isDuplicate }, commandType: System.Data.CommandType.Text);
        }

        public void UpdateHierarchyInfo(int numericId, string companyCode, string parentId, string groupId, string groupRisk, int isDuplicate = 0, string groupEntityOf = null, string relationship = null, string flagType = null)
        {
            const string sql = @"
                UPDATE customermaster SET 
                    company_code = COALESCE(@companyCode, company_code), 
                    parent_id = COALESCE(@parentId, parent_id), 
                    group_id = @groupId, 
                    group_risk = @groupRisk, 
                    group_entity_of = @groupEntityOf,
                    is_duplicate = @isDuplicate,
                    relationship = COALESCE(@relationship, relationship),
                    flagtype = COALESCE(@flagType, flagtype)
                WHERE cust_ref_id = (SELECT cust_id FROM customercase WHERE id = @id);

                UPDATE customercase SET 
                    is_duplicate = @isDuplicate 
                WHERE id = @id;";

            Execute(sql, new { id = numericId, companyCode, parentId, groupId, groupRisk, isDuplicate, groupEntityOf, relationship, flagType }, commandType: System.Data.CommandType.Text);
        }

        public IEnumerable<CustomerCaseDTO> GetAllGroupEntities(int clientId)
        {
            const string sql = @"
                SELECT ca.id as Id, cu.group_id as GroupId, cu.group_risk as GroupRisk, cu.group_entity_of as GroupEntityof 
                FROM customercase ca 
                JOIN customermaster cu ON ca.cust_master_id = cu.id
                WHERE cu.group_id IS NOT NULL AND cu.group_id != '' AND ca.is_deleted = 0 AND cu.is_deleted = 0 AND ca.client_id = @clientId
                ORDER BY ca.id DESC";
            using (var conn = GetConnections())
            {
                var result = conn.Query<dynamic>(sql, new { clientId }, commandType: System.Data.CommandType.Text);
                var list = new List<CustomerCaseDTO>();
                foreach(var r in result)
                {
                    list.Add(new CustomerCaseDTO 
                    {
                        Id = System.Convert.ToInt32(r.Id),
                        GroupId = r.GroupId?.ToString(),
                        GroupRisk = r.GroupRisk?.ToString(),
                        GroupEntityof = r.GroupEntityof?.ToString()
                    });
                }
                return list;
            }
        ...
        public IEnumerable<CustomerCaseDTO> GetGroupMembers(string groupId)
        {
            const string sql = @"
                SELECT ca.id as Id, ca.cust_id as CustomerId, TRIM(COALESCE(NULLIF(TRIM(CONCAT(COALESCE(cu.fname, ''), ' ', COALESCE(cu.lname, ''))), ''), NULLIF(TRIM(cu.lname), ''))) as FullName, cu.group_risk as GroupRisk, cu.group_entity_of as GroupEntityof 
                FROM customercase ca 
                JOIN customermaster cu ON ca.cust_master_id = cu.id
                WHERE FIND_IN_SET(@groupId, cu.group_id) > 0 AND ca.is_deleted = 0 AND cu.is_deleted = 0
                ORDER BY ca.id DESC";
            using (var conn = GetConnections())
            {
                var result = conn.Query<dynamic>(sql, new { groupId }, commandType: System.Data.CommandType.Text);
                var list = new List<CustomerCaseDTO>();
                foreach(var r in result)
                {
                    list.Add(new CustomerCaseDTO 
        ...
        public bool AddGroupMember(string groupId, string customerId, string groupName = null, string groupRisk = null)
        {
            using (var conn = GetConnections())
            {
                if (string.IsNullOrEmpty(groupName))
                {
                    // Find the group's name and risk from any existing member of the group
                    var groupInfo = conn.QueryFirstOrDefault<dynamic>(
                        "SELECT group_entity_of as GroupName, group_risk as GroupRisk FROM customermaster WHERE FIND_IN_SET(@groupId, group_id) > 0 LIMIT 1",
                        new { groupId });
                    
                    groupName = groupInfo?.GroupName ?? "Unknown Group";
                    groupRisk = groupInfo?.GroupRisk ?? "Low Risk";
                }

                var master = conn.QueryFirstOrDefault<dynamic>(
                    "SELECT id, group_id, group_entity_of, group_risk FROM customermaster WHERE id = (SELECT cust_master_id FROM customercase WHERE cust_id = @customerId LIMIT 1)",
                    new { customerId });
                
                if (master == null) return false;

                var ids = string.IsNullOrEmpty((string)master.group_id) ? new List<string>() : new List<string>(((string)master.group_id).Split(','));
                var names = string.IsNullOrEmpty((string)master.group_entity_of) ? new List<string>() : new List<string>(((string)master.group_entity_of).Split(','));
                var risks = string.IsNullOrEmpty((string)master.group_risk) ? new List<string>() : new List<string>(((string)master.group_risk).Split(','));
                ...
                string newRisks = string.Join(",", risks);

                const string sql = @"
                    UPDATE customermaster 
                    SET group_id = @newIds, group_entity_of = @newNames, group_risk = @newRisks
                    WHERE id = @id";
                
                int rows = conn.Execute(sql, new { newIds, newNames, newRisks, id = master.id }, commandType: System.Data.CommandType.Text);
                return true;
            }
        }

        public bool RemoveGroupMember(string groupId, string customerId)
        {
            using (var conn = GetConnections())
            {
                var master = conn.QueryFirstOrDefault<dynamic>(
                ...
                if (master == null || string.IsNullOrEmpty(master.group_id)) return false;

                var ids = new List<string>(((string)master.group_id).Split(','));
                var names = string.IsNullOrEmpty((string)master.group_entity_of) ? new List<string>() : new List<string>(((string)master.group_entity_of).Split(','));
                var risks = string.IsNullOrEmpty((string)master.group_risk) ? new List<string>() : new List<string>(((string)master.group_risk).Split(','));
                ...
                if (idx < risks.Count) risks.RemoveAt(idx);
        ...
        public ServiceResponse<string> CreateShareholdersData(ShareholderDTO _shareholderDTO)
        {
            ...
            try
            {
                ...
                parameters.Add("@p_placeofbirth", _shareholderDTO.PlaceOfBirth);
                ...
                parameters.Add("@p_isduplicate", _shareholderDTO.IsDuplicate);
                parameters.Add("@p_placeofbirth", _shareholderDTO.PlaceOfBirth);
        ...
        public ServiceResponse<bool> SaveCaseDraft(int clientId, int userId, string draftId, string draftName, int nodeCount, int edgeCount, string draftJson)
        {
            ServiceResponse<bool> serviceResponse = new ServiceResponse<bool>();
            try
            {
                string createTableSql = @"CREATE TABLE IF NOT EXISTS cs_casestudio_draft (
                    ...
                    PRIMARY KEY (client_id, user_id, draft_id)
                );";
                Execute(createTableSql, null, commandType: CommandType.Text);

                string insertSql = @"INSERT INTO cs_casestudio_draft (client_id, user_id, draft_id, draft_name, node_count, edge_count, draft_json, updated_on) 
                    VALUES (@ClientId, @UserId, @DraftId, @DraftName, @NodeCount, @EdgeCount, @DraftJson, @UpdatedOn) 
                    ON DUPLICATE KEY UPDATE draft_name = @DraftName, node_count = @NodeCount, edge_count = @EdgeCount, draft_json = @DraftJson, updated_on = @UpdatedOn;";
                    
                DynamicParameters parameters = new DynamicParameters();
                parameters.Add("@ClientId", clientId);
                parameters.Add("@UserId", userId);
                parameters.Add("@DraftId", draftId);
                parameters.Add("@DraftName", draftName);
                parameters.Add("@NodeCount", nodeCount);
                parameters.Add("@EdgeCount", edgeCount);
                parameters.Add("@DraftJson", draftJson);
                parameters.Add("@UpdatedOn", DateTime.UtcNow);

                Execute(insertSql, parameters, commandType: CommandType.Text);
                serviceResponse.Result = true;
                serviceResponse.Status = StaticResource.SuccessStatusCode;
                serviceResponse.Message = "Draft saved successfully.";
            }
            catch (Exception ex)
            {
        ...
        public ServiceResponse<IEnumerable<CaseStudioDraftSummaryDTO>> GetCaseDraftSummaries(int clientId, int userId)
        {
            ServiceResponse<IEnumerable<CaseStudioDraftSummaryDTO>> serviceResponse = new ServiceResponse<IEnumerable<CaseStudioDraftSummaryDTO>>();
            try
            {
                string createTableSql = @"CREATE TABLE IF NOT EXISTS cs_casestudio_draft (
                    client_id INT NOT NULL,
                ...
                string selectSql = "SELECT draft_id AS DraftId, draft_name AS DraftName, node_count AS NodeCount, edge_count AS EdgeCount, updated_on AS UpdatedOn FROM cs_casestudio_draft WHERE client_id = @ClientId AND user_id = @UserId ORDER BY updated_on DESC;";
                DynamicParameters parameters = new DynamicParameters();
                parameters.Add("@ClientId", clientId);
                parameters.Add("@UserId", userId);

                var summaries = Get<CaseStudioDraftSummaryDTO>(selectSql, parameters, commandType: CommandType.Text);
                serviceResponse.Result = summaries;
                serviceResponse.Status = StaticResource.SuccessStatusCode;
            }
            catch (Exception ex)
            {
        ...
        public ServiceResponse<string> GetCaseDraft(int clientId, int userId, string draftId)
        {
            ServiceResponse<string> serviceResponse = new ServiceResponse<string>();
            try
            {
                ...
                string selectSql = "SELECT draft_json FROM cs_casestudio_draft WHERE client_id = @ClientId AND user_id = @UserId AND draft_id = @DraftId;";
                DynamicParameters parameters = new DynamicParameters();
                parameters.Add("@ClientId", clientId);
                parameters.Add("@UserId", userId);
                parameters.Add("@DraftId", draftId);

                string draftJson = GetFirstOrDefault<string>(selectSql, parameters, commandType: CommandType.Text);
                serviceResponse.Result = draftJson;
                serviceResponse.Status = StaticResource.SuccessStatusCode;
            }
            catch (Exception ex)
            {
        ...
        public ServiceResponse<bool> DeleteCaseDraft(int clientId, int userId, string draftId)
        {
            ServiceResponse<bool> serviceResponse = new ServiceResponse<bool>();
            try
            {
                ...
                string deleteSql = "DELETE FROM cs_casestudio_draft WHERE client_id = @ClientId AND user_id = @UserId AND draft_id = @DraftId;";
                DynamicParameters parameters = new DynamicParameters();
                parameters.Add("@ClientId", clientId);
                parameters.Add("@UserId", userId);
                parameters.Add("@DraftId", draftId);

                Execute(deleteSql, parameters, commandType: CommandType.Text);
                serviceResponse.Result = true;
                serviceResponse.Status = StaticResource.SuccessStatusCode;
                serviceResponse.Message = "Draft deleted successfully.";
            }
            catch (Exception ex)
            {
                serviceResponse.Result = false;
        ...
        public ServiceResponse<bool> RenameCaseDraft(int clientId, int userId, string draftId, string newDraftName)
        {
            ServiceResponse<bool> serviceResponse = new ServiceResponse<bool>();
            try
            {
                ...
                string updateSql = "UPDATE cs_casestudio_draft SET draft_name = @DraftName, updated_on = @UpdatedOn WHERE client_id = @ClientId AND user_id = @UserId AND draft_id = @DraftId;";
                DynamicParameters parameters = new DynamicParameters();
                parameters.Add("@ClientId", clientId);
                parameters.Add("@UserId", userId);
                parameters.Add("@DraftId", draftId);
                parameters.Add("@DraftName", newDraftName);
                parameters.Add("@UpdatedOn", DateTime.UtcNow);

                Execute(updateSql, parameters, commandType: CommandType.Text);
                serviceResponse.Result = true;
                serviceResponse.Status = StaticResource.SuccessStatusCode;
                serviceResponse.Message = "Draft renamed successfully.";
            }
            catch (Exception ex)
            {
                serviceResponse.Result = false;
