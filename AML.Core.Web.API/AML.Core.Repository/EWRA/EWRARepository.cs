    ...
    public class EWRARepository : BaseRepository, IEWRARepository
    {
        ...
        public ServiceResponse<string> SaveCustomerProfile(EWRAModelDTO model)
        {
            ...
            try
            {
                ...
                {
                    custProfData += model.EWRACustomerVolume[i].CategoryId + "Ø" + model.EWRACustomerVolume[i].TypeId + "Ø" + model.EWRACustomerVolume[i].High + "Ø" +
                        model.EWRACustomerVolume[i].Medium + "Ø" + model.EWRACustomerVolume[i].Low + "Ø" + model.EWRACustomerVolume[i].Total + "Ø" +
                        model.EWRACustomerVolume[i].RiskScore + "Ø" + model.EWRACustomerVolume[i].Status + "¥";
                    index++;
                }
                for (int i = 0; i < model.EWRACustomerCount.Count; i++)
                {
                    custProfData += model.EWRACustomerCount[i].CategoryId + "Ø" + model.EWRACustomerCount[i].TypeId + "Ø" + model.EWRACustomerCount[i].High + "Ø" +
                        model.EWRACustomerCount[i].Medium + "Ø" + model.EWRACustomerCount[i].Low + "Ø" + model.EWRACustomerCount[i].Total + "Ø" +
                        model.EWRACustomerCount[i].RiskScore + "Ø" + model.EWRACustomerCount[i].Status + "¥";
                    index++;
                }
                for (int i = 0; i < model.EWRACustomerTransaction.Count; i++)
                {
                    custProfData += model.EWRACustomerTransaction[i].CategoryId + "Ø" + model.EWRACustomerTransaction[i].TypeId + "Ø" + model.EWRACustomerTransaction[i].High + "Ø" +
                        model.EWRACustomerTransaction[i].Medium + "Ø" + model.EWRACustomerTransaction[i].Low + "Ø" + model.EWRACustomerTransaction[i].Total + "Ø" +
                        model.EWRACustomerTransaction[i].RiskScore + "Ø" + model.EWRACustomerTransaction[i].Status + "¥";
                    index++;
                }
                ...
                for (int i = 0; i < model.EWRACounterPartyType.Count; i++)
                {
                    for (int j = 0; j < model.EWRACounterPartyType[i].EWRACounterPartyList.Count; j++)
                    {
                        ...
                        counterData +=
                        model.EWRACounterPartyType[i].EWRACounterPartyList[j].CounterParty + "Ø" +
                        model.EWRACounterPartyType[i].EWRACounterPartyList[j].selectedRiskId + "Ø" +
                        model.EWRACounterPartyType[i].EWRACounterPartyList[j].selectedRisk + "Ø" +
                        model.EWRACounterPartyType[i].EWRACounterPartyList[j].Country + "Ø" +
                        model.EWRACounterPartyType[i].EWRACounterPartyList[j].CountryId + "Ø" +
                        model.EWRACounterPartyType[i].EWRACounterPartyList[j].CountryRiskScore + "Ø" +
                        model.EWRACounterPartyType[i].EWRACounterPartyList[j].CountryRiskRating + "Ø" +
                        model.EWRACounterPartyType[i].EWRACounterPartyList[j].ExpenseAmount + "Ø" +
                        model.EWRACounterPartyType[i].EWRACounterPartyList[j].OverrideApplied + "¥";
                        counterIndex++;
                    }
                }
                ...
                for (int i = 0; i < model.EWRAProductCategory.Count; i++)
                {
                    ...
                    {
                        productData += model.EWRAProductCategory[i].EWRAProductList[j].Product + "Ø" + model.EWRAProductCategory[i].EWRAProductList[j].ProductId + "Ø" + model.EWRAProductCategory[i].EWRAProductList[j].High + "Ø" + model.EWRAProductCategory[i].EWRAProductList[j].Medium + "Ø" + model.EWRAProductCategory[i].EWRAProductList[j].Low
                             + "Ø" + model.EWRAProductCategory[i].EWRAProductList[j].Total + "Ø" + model.EWRAProductCategory[i].EWRAProductList[j].RiskScore + "¥";
                        productIndex++;
                    }
                }
                ...
                {
                    jurData += model.EWRAJurisdiction[i].Country + "Ø" + (model.EWRAJurisdiction[i].CountryId == 0 ? 0 : model.EWRAJurisdiction[i].CountryId) + "Ø" + model.EWRAJurisdiction[i].CountryRiskScore + "Ø" + model.EWRAJurisdiction[i].CountryRiskRating + "Ø" + model.EWRAJurisdiction[i].CountryRiskLevel + "Ø" + model.EWRAJurisdiction[i].LedgerAmount + "¥";
                    jurIndex++;
                }
                ...
                {
                    deliveryData += model.EWRADeliveryType[i].DeliveryTypeId + "Ø" + model.EWRADeliveryType[i].DeliveryType + "Ø" + model.EWRADeliveryType[i].Cash + "Ø" + model.EWRADeliveryType[i].Bank + "Ø" + model.EWRADeliveryType[i].RiskLevel + "Ø" + model.EWRADeliveryType[i].RiskScore + "¥";
                    deliveryIndex++;
                }
            ...
            try
            {
                ...
                for (int i = 0; i < model.EWRAQualitativeModel.Count; i++)
                {
                    for (int j = 0; j < model.EWRAQualitativeModel[i].EWRARiskModel.Count; j++)
                    {
                        ...
                        {
                            inherentData += model.EWRAQualitativeModel[i].RiskTypeId + "Ø" + model.EWRAQualitativeModel[i].RiskType + "Ø" +
                                model.EWRAQualitativeModel[i].EWRARiskModel[j].DescriptionId + "ØØ" + /*model.EWRAQualitativeModel[i].EWRARiskModel[j].RiskDescription*/
                                model.EWRAQualitativeModel[i].EWRARiskModel[j].InherentRiskModelData[x].InherentRiskId + "ØØ" + /*model.EWRAQualitativeModel[i].EWRARiskModel[j].InherentRiskModelData[x].InherentRiskDescription + "Ø" +*/
                                model.EWRAQualitativeModel[i].EWRARiskModel[j].InherentRiskModelData[x].InherentImpactId + "Ø" + model.EWRAQualitativeModel[i].EWRARiskModel[j].InherentRiskModelData[x].InherentImpact + "Ø" +
                                model.EWRAQualitativeModel[i].EWRARiskModel[j].InherentRiskModelData[x].InherentLikelihoodId + "Ø" + model.EWRAQualitativeModel[i].EWRARiskModel[j].InherentRiskModelData[x].InherentLikelihood + "Ø" +
                                model.EWRAQualitativeModel[i].EWRARiskModel[j].InherentRiskModelData[x].InherentRiskRating + "Ø" + model.EWRAQualitativeModel[i].EWRARiskModel[j].InherentRiskModelData[x].InherentRiskScore + "¥";
                            inherentIndex++;
                        }
                        for (var y = 0; y < model.EWRAQualitativeModel[i].EWRARiskModel[j].KeyControlModelData.Count; y++)
                        {
                            keyData += model.EWRAQualitativeModel[i].RiskTypeId + "ØØ" + /*model.EWRAQualitativeModel[i].RiskType + "Ø" +*/
                                model.EWRAQualitativeModel[i].EWRARiskModel[j].DescriptionId + "ØØ" + /*model.EWRAQualitativeModel[i].EWRARiskModel[j].RiskDescription + "Ø" +*/
                                model.EWRAQualitativeModel[i].EWRARiskModel[j].KeyControlModelData[y].KeyId + "ØØ" + /*model.EWRAQualitativeModel[i].EWRARiskModel[j].KeyControlModelData[y].KeyControl + "Ø" + */
                                model.EWRAQualitativeModel[i].EWRARiskModel[j].KeyControlModelData[y].SelectedKey + "ØØ" +
                                model.EWRAQualitativeModel[i].EWRARiskModel[j].ResidualRiskModelData[y].ResidualRiskScore + "¥";
                            keyIndex++;

                        }
                       
                    }
                    Data += model.EWRAQualitativeModel[i].RiskTypeId + "Ø" + model.EWRAQualitativeModel[i].RiskType + "Ø" +
                        model.EWRAQualitativeModel[i].InherentImpactAvg + "Ø" + model.EWRAQualitativeModel[i].InherentLikelihoodAvg + "Ø" + model.EWRAQualitativeModel[i].InherentRiskRatingAvg + "Ø" + model.EWRAQualitativeModel[i].InherentRiskScoreAvg + "Ø" +
                        model.EWRAQualitativeModel[i].KeyControlRatingAvg + "Ø" + model.EWRAQualitativeModel[i].KeyControlScoreAvg + "Ø" +
                        model.EWRAQualitativeModel[i].ResidualRiskRatingAvg + "Ø" + model.EWRAQualitativeModel[i].ResidualRiskScoreAvg + "¥";
                    Index++;
                }
                parameters.Add("@p_dateOfAssessment", model.DateOfAssessment);
        ...
        public ServiceResponse<string> GetPendingEWRA(int type, int userId,int clientId)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        response += item.QuantitativeId + "Ø" + item.QuantitativeName + "¥";
                    }
                }
                if (response == null)
                {
