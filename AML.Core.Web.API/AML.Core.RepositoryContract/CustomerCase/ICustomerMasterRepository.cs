...
namespace AML.Core.RepositoryContract.CustomerCase
{
    public interface ICustomerMasterRepository: IBaseRepository
    {
        ...
        ServiceResponse<int> UpdateCustomerMaster(CustomerMasterDTO _CustomerMasterDTO);
        ...
        void TouchUpdatedOn(int numericId, int updatedBy);
        void UpdateGroupInfo(int numericId, string groupId, string groupRisk, int isDuplicate = 0, string groupEntityOf = null);
        void UpdateHierarchyInfo(int numericId, string companyCode, string parentId, string groupId, string groupRisk, int isDuplicate = 0, string groupEntityOf = null, string relationship = null, string flagType = null);
        ...
        ServiceResponse<bool> SaveCaseDraft(int clientId, int userId, string draftId, string draftName, int nodeCount, int edgeCount, string draftJson);
        ServiceResponse<IEnumerable<CaseStudioDraftSummaryDTO>> GetCaseDraftSummaries(int clientId, int userId);
        ServiceResponse<string> GetCaseDraft(int clientId, int userId, string draftId);
        ServiceResponse<bool> DeleteCaseDraft(int clientId, int userId, string draftId);
        ServiceResponse<bool> RenameCaseDraft(int clientId, int userId, string draftId, string newDraftName);
        
        IEnumerable<CustomerCaseDTO> GetGroupMembers(string groupId);
        bool AddGroupMember(string groupId, string customerId, string groupName = null, string groupRisk = null);
        bool RemoveGroupMember(string groupId, string customerId);
    }
}
