...
namespace AML.Core.Service.Common
{
    public class CommonService : BaseService, ICommonService
    {
        ...
        private string c6WebBaseURL = string.Empty;
        ...
        public CommonService(IMapper mapper, ICommonRepository commonRepository, IDigiApiUserService digiApiUserService, ICountryRepository countryRepository, IConfiguration configuration, IHostingEnvironment env, ICustomerCaseService customerCaseService, ICustomerMasterRepository customerMasterRepository, IFreeSourceRepository freeSourceRepository, IUserRepository userRepository, ITransactionScreeningService transactionScreeningService, ICaseCommentService caseCommentService, IHttpContextAccessor httpContextAccessor) : base(commonRepository, configuration)
        {
            ...
            c6WebBaseURL = configuration.GetSection("C6BaseApiUrl").GetSection("webBaseUrl").Value;
            _webPassword = configuration.GetSection("C6BaseApiUrl").GetSection("webPassword").Value;
            _webUsername = configuration.GetSection("C6BaseApiUrl").GetSection("webUsername").Value;
		...
		public async Task<CustomerCaseDTO> C6ScreeningCall(CustomerCaseDTO _CustomerCaseDTO, string baseUrl, string callFrom = null, string emailBody = null, int checkThreshold = -1, string cid = "")
		{
			...
			_CustomerCaseDTO.Id = _customerCaseService.GetCaseId(cid);
        ...
        private async Task<CustomerCaseDTO> FreeSourceScreening(CustomerCaseDTO _CustomerCaseDTO, string baseUrl, string schedulerRunId, string callFrom = null, string emailBody = null)
        {
            ...
            if (apiResp.IsNotNullOrEmpty() && apiResp.data.IsNotNullOrEmpty())
            {
                ...
                if (apiResp.data.matchtype != "KYC6")
                {
                    ...
                    _CustomerCaseDTO.IsMatched =
     (_CustomerCaseDTO.Status == 0 || _CustomerCaseDTO.Status == 6) ? 1 : 0;
                    //_CustomerCaseDTO.CaseChangeStatus = apiResp.data.matchtype;
                    _CustomerCaseDTO.CaseChangeStatus =
               !string.IsNullOrEmpty(apiResp?.data?.matchtype)
                   ? (
                       apiResp.data.Status == "D"
                           ? apiResp.data.matchtype + "-Former"
                           : apiResp.data.matchtype
                     )
                   : string.Empty;
        ...
        {
            var customerIdType = (_CustomerCaseDTO?.CustomerIdType ?? "").ToLowerInvariant();

            var hasPassport = customerIdType.Contains("passport");
            var hasEmiratesId = customerIdType.Contains("emirates id");
            C6ScreeningRQ screeningrq = new C6ScreeningRQ
            {
                ...
                idtype = string.IsNullOrWhiteSpace(customerIdType)
                ? null
                : hasPassport && hasEmiratesId
                    ? "other"
                    : hasPassport
                        ? "passport number"
                        : "emirates id",

                            idnumber = string.IsNullOrWhiteSpace(customerIdType)
                ? null
                : hasPassport && hasEmiratesId
                    ? _CustomerCaseDTO.PassportId
                    : hasPassport
                        ? _CustomerCaseDTO.PassportId
                        : _CustomerCaseDTO.EmiratesIdNumber,
                placeofbirth = new List<string>(),
            ...
            if (_CustomerCaseDTO.DOB != DateTime.MinValue &&
    _CustomerCaseDTO.DOB != new DateTime(1900, 1, 1))
            {
            ...
            {
                screeningrq.dob = null;
                screeningrq.dobMatching = null;
            }
            ...
            if (_CustomerCaseDTO.Nationality.IsNotNullOrEmpty())
            {
                ...
                if (parsedCountry.IsNotNullOrEmpty())
                {
                    screeningrq.countries.Add(parsedCountry);
                }
            }
            if (_CustomerCaseDTO.PlaceOfBirth.IsNotNullOrEmpty())
            {
                string parsedPlaceofbirth = GetTwoCharName(_CustomerCaseDTO.PlaceOfBirth);

                if (parsedPlaceofbirth.IsNotNullOrEmpty())
                {
                    screeningrq.placeofbirth.Add(parsedPlaceofbirth);
                }
            }
        ...
        private WebScreeningRQ GetWebScreeningRequest(CustomerCaseDTO _CustomerCaseDTO)
        {

            WebScreeningRQ screeningrq = new WebScreeningRQ
            {
                
                searchName = string.Concat(_CustomerCaseDTO.FirstName, " ", _CustomerCaseDTO.MiddleName, " ", _CustomerCaseDTO.LastName, " ", _CustomerCaseDTO.onb_name),
                threshold = _CustomerCaseDTO.Threshold,
            };
        ...
        private async Task<CustomerCaseDTO> C6Screening(CustomerCaseDTO _CustomerCaseDTO, string baseUrl, string callFrom = null, string emailBody = null, int cid = -1)
        {
            ...
            WebScreeningRQ WEBscreeningRequest = GetWebScreeningRequest(_CustomerCaseDTO);
            ...
            //try
            //{
                // A5: do NOT log full screening request (contains PII — name/DOB/nationality).
                // Log only a structural summary that's useful for debugging.
                log.Debug($"Screening request prepared: corporate={corporate}, hasName={(screeningRequest?.name?.Length > 0)}, threshold={_CustomerCaseDTO.Threshold}, clientId={_CustomerCaseDTO.ClientId}");
            ...
            checkThreshold = _CustomerCaseDTO.Threshold;
                ...
                if (_CustomerCaseDTO.IsWhiteListed != "YES")
                {
                    ...
                    var screeningTask = AMLUtility.C6ScreeningAPICall(screeningRequest,_CustomerCaseDTO.CustomerType == "C" ? ScreeningService.C6SCREENINGCORPORATE : ScreeningService.C6SCREENING,baseUrl);

                    var webSearchTask = AMLUtility.WEBScreeningAPICall(WEBscreeningRequest,ScreeningService.C6WEBSEARCH,c6WebBaseURL,_webUsername,_webPassword);

                    await Task.WhenAll(screeningTask, webSearchTask);

                    string screeningResponse = null;
                    ...
                    try
                    {
                        screeningResponse = await screeningTask;
                    }
                    catch (Exception ex)
                    {
                        log.Error($"Screening API Error: {ex}");
                    }

                    try
                    {
                        webSearchResponse = await webSearchTask;
                    }
                    catch (Exception ex)
                    {
                        log.Error($"Web Search API Error: {ex}");
                    }
                    log.Info($"Got response {(screeningResponse.Length > 100 ? screeningResponse.Substring(0, 100) + "..." : screeningResponse)}");

                        if (screeningResponse.IsNotNullOrEmpty())
                        {
                            apiResp = JsonConvert.DeserializeObject<C6ScreeningRS>(screeningResponse);
                        ...
                        else if (screeningResponse == null)
                        {
                    ...
                    if (!string.IsNullOrWhiteSpace(webSearchResponse))
                    {
                        var webResponseObj = JsonConvert.DeserializeObject<WebSearchRS>(webSearchResponse);
                        webResults = webResponseObj?.results ?? new List<WebSearchResult>();
                    }
                }
                }
                catch (Exception Ex)
                {
                    log.Error(Ex, "C6 screening call failed");
                    // R2: when the screening provider is unreachable, do NOT silently auto-approve.
                    // Mark the case Pending (0) so a human must review/retry instead of letting it pass.
                    _CustomerCaseDTO.Source = "C6_UNAVAILABLE";
                    _CustomerCaseDTO.Status = 0; // Pending
                    return _CustomerCaseDTO;
                }
            //         }
            //         catch (Exception ex)
            ...
            // R5: defensively guard the entire chain against any null link.
            bool blacklistedOrKYCHit = IsBlackListed.exists ||
                (
                    apiResp != null &&
                    apiResp.users != null &&
                    apiResp.users.results != null &&
                    apiResp.users.results.matches != null &&
                    apiResp.users.results.matchCount > 0
                ) ||
                (
                    webResults != null && webResults.Any(w => w.matchScore >= checkThreshold)
                );
           ...
           if (blacklistedOrKYCHit) {
            ...
            //{
                // R5: null-safe access throughout
                var matches = apiResp?.users?.results?.matches;
                var matchCount = matches?.Count ?? 0;
                var topMatch = matches?.FirstOrDefault();
                _CustomerCaseDTO.Source = "KYC6";
                _CustomerCaseDTO.MatchScore = (matchCount > 0 ? (topMatch?.score ?? 0) : 0);
                _CustomerCaseDTO.SourceUniqueId = matchCount > 0 ? Convert.ToString(topMatch?.qrCode) : null;
                _CustomerCaseDTO.Status = _CustomerCaseDTO.MatchScore.IsNotNullOrEmpty() ? (_CustomerCaseDTO.MatchScore < checkThreshold ? 5 : 0) : 5;
            var firstMatch = topMatch;

            if(firstMatch?.datasets != null)
            {
                _CustomerCaseDTO.CaseChangeStatus = string.Join(", ",
                    firstMatch.datasets.Select(d =>
            ...
            var topWebMatch = webResults?.OrderByDescending(x => x.matchScore).FirstOrDefault();
            int webTopScore = topWebMatch?.matchScore ?? 0;

            // R5: same null-safe pattern in the blacklisted branch
            var bl_matches = apiResp?.users?.results?.matches;
            var bl_topMatch = bl_matches?.FirstOrDefault();
            var bl_matchCount = bl_matches?.Count ?? 0;

            // Default KYC6 assignment
            _CustomerCaseDTO.Source = "KYC6";
            _CustomerCaseDTO.MatchScore = bl_matchCount > 0 ? (bl_topMatch?.score ?? 0) : webTopScore;
            _CustomerCaseDTO.SourceUniqueId = bl_matchCount > 0 ? Convert.ToString(bl_topMatch?.qrCode) : null;
            _CustomerCaseDTO.Status = _CustomerCaseDTO.MatchScore.IsNotNullOrEmpty() ? (_CustomerCaseDTO.MatchScore < checkThreshold ? 5 : 0) : 5;
            _CustomerCaseDTO.CaseChangeStatus = "KYC6";
            ...
            if (IsBlackListed.response != null && IsBlackListed.response.Count() != 0)
            {
                ...
                _CustomerCaseDTO.MatchScore = IsBlackListed.exists ? 100 : (bl_matchCount > 0 ? (bl_topMatch?.score ?? 0) : webTopScore);
                _CustomerCaseDTO.SourceUniqueId = IsBlackListed.exists ? IsBlackListed.response[0].TYPE : (bl_matchCount > 0 ? Convert.ToString(bl_topMatch?.qrCode) : null);
                _CustomerCaseDTO.Status = _CustomerCaseDTO.MatchScore.IsNotNullOrEmpty() ? (_CustomerCaseDTO.MatchScore < checkThreshold ? 5 : 0) : 5;
                _CustomerCaseDTO.CaseChangeStatus = IsBlackListed.exists ? (IsBlackListed.response[0].STATUS == "D" ? IsBlackListed.response[0].TYPE + "-Former" : IsBlackListed.response[0].TYPE) : "KYC6";
            }
            else if (IsBlackListed.response2 != null && IsBlackListed.response2.Count() != 0)
            {
                _CustomerCaseDTO.Source = IsBlackListed.exists ? (IsBlackListed.response2[0].TYPE) : "KYC6";
                _CustomerCaseDTO.MatchScore = IsBlackListed.exists ? 100 : (bl_matchCount > 0 ? (bl_topMatch?.score ?? 0) : webTopScore);
                _CustomerCaseDTO.SourceUniqueId = IsBlackListed.exists ? IsBlackListed.response2[0].TYPE : (bl_matchCount > 0 ? Convert.ToString(bl_topMatch?.qrCode) : null);
                _CustomerCaseDTO.Status = _CustomerCaseDTO.MatchScore.IsNotNullOrEmpty() ? (_CustomerCaseDTO.MatchScore < checkThreshold ? 5 : 0) : 5;
                _CustomerCaseDTO.CaseChangeStatus = IsBlackListed.exists ? (IsBlackListed.response2[0].STATUS == "D" ? IsBlackListed.response2[0].TYPE + "-Former"  : IsBlackListed.response2[0].TYPE) : "KYC6";
            }
            ...
            try
            {
                ...
                if (bl_matches != null)
                {
                    foreach (DTO.DTO.CustomerScreening.Match item in bl_matches)
                    {
            ...
            var webRecords = new List<WEBLOGMATCH>();
                var stat = 1;
            ...
            //{
                ...
                if (apiResp?.users?.results?.matches != null)
                {
                ...
                if (item.IsNotNullOrEmpty())
                    ...
                    //                  d.StartsWith("SAN", StringComparison.OrdinalIgnoreCase) ? "SAN" : d)
                    //     .Distinct())
                    // : "--";
                    ...
                    //List<WebListModel> webResultList = new List<WebListModel>();

                    //if (apiResp.users.results.webSearches != null &&
                    //    apiResp.users.results.webSearches.matches != null)
                    //{
                    //    foreach (var item in apiResp.users.results.webSearches.matches)
                    //    {
                    //        WebListModel webResult = new WebListModel();

                    //        webResult.shortdescritption = item.shortDescription;
                    //        webResult.url = item.url;

                    //        webResultList.Add(webResult);
                    //    }
                    //}
        ...
        log.Debug("Update customer case table with details after screening");
            ...
            if (blacklistedOrKYCHit)
            {
                ...
                //{
                 ...
                 webRecords = webResults
                    .Select(x => new WEBLOGMATCH
                    {
                        SHORTDESCRIPTION = x.shortDescription,
                        URL = x.URL,
                        TITLE=x.title,
                        MATCHSCORE=x.matchScore,
                        MATCHPRESENT =x.matchPercent,
                        MATCHEDNAMETERMS =x.matchedNameTerms,
                        CRIMEKEYWORDSFOUND=x.crimeKeywordsFound,
                        ENGINE=x.engine
                ...
                //insert caselog in mongodb
                if (stat >= 1 || (webRecords != null && webRecords.Count > 0) || IsBlackListed.exists)
                {
                    CASELOG modelCaseLog = new CASELOG
                    {
                        CASEID = _CustomerCaseDTO.Id.ToString(),
...
//#endif
        ...
        public bool UpdateCaseRemark(int caseId, List<DataListModel> model)
        {
            ...
            for (int i = 0; i < model.Count; i++)
            {
                ...
                var matchrecords = new MatchRecordsDTO();
                matchrecords.MATCHUID = model[i].matchuid;
                matchrecords.MATCHIDNO = model[i].matchidno;
