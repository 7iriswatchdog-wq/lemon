...
namespace AML.Core.Service.ProliferationFinance
{
    public class ProliferationFinanceService : IProliferationFinanceService
    {
        ...
        public ServiceResponse<string> SearchMilitaryGoods(string keyword)
        {
            try
            {
                string filePath = Path.Combine(Directory.GetCurrentDirectory(), "Files", "Stand-alone-Munitions-List-2025-Corr.pdf");
            ...
            catch (Exception ex)
            {
                ...
                {
                    Status = 500,
                    Message = "Error searching Military Goods PDF: " + ex.Message
                };
            }
        }
        ...
        public ServiceResponse<bool> SyncSearchResultsToMongo(int caseId, string productName, List<UAEControlListDTO> chemicalHits, string pdfHits, string searchType = "Non-Chemical")
        {
            try
            {
        ...
        public ServiceResponse<bool> UpdateMongoHitDecision(int caseId, string searchType, string decision, string remarks)
        {
            var success = _mongoRepository.UpdateHitDecision(caseId, searchType, decision, remarks);
            return new ServiceResponse<bool> { Status = success ? 200 : 500, Result = success };
        }
