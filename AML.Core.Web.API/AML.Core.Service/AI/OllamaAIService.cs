...
namespace AML.Core.Service.AI
{
    public class OllamaAIService : BaseService, IAIService
    {
        ...
        private readonly MockAIService _fallbackMock;

        public OllamaAIService(IConfiguration configuration) : base(configuration)
        {
            _fallbackMock = new MockAIService(configuration);
...
- Always prioritize the provided [SYSTEM KNOWLEDGE BASE] for procedural rules.";
        ...
        public async Task<string> GetIntelligentReplyAsync(string prompt, string context)
        {
            try
            {
                ...
                return await _fallbackMock.GetIntelligentReplyAsync(prompt, context);
            }
            catch
            {
                return await _fallbackMock.GetIntelligentReplyAsync(prompt, context);
        ...
        private async IAsyncEnumerable<string> GetIntelligentReplyStreamInternalAsync(string prompt, string context, List<ChatMessageDTO> history)
        {
        ...
        public async IAsyncEnumerable<string> GetIntelligentReplyStreamAsync(string prompt, string context, List<ChatMessageDTO> history = null, bool isNewChat = false)
        {
            ...
            var enumerator = GetIntelligentReplyStreamInternalAsync(prompt, context, history).GetAsyncEnumerator();
            bool failed = false;
            ...
            if (failed)
            {
                await foreach (var part in _fallbackMock.GetIntelligentReplyStreamAsync(prompt, context, history, false))
                {
                    yield return part;
                }
            }
        ...
        private async IAsyncEnumerable<string> GetGeneralReplyStreamInternalAsync(string prompt, string moduleContext, List<ChatMessageDTO> history)
        {
        ...
        public async IAsyncEnumerable<string> GetGeneralReplyStreamAsync(string prompt, string moduleContext, List<ChatMessageDTO> history = null, bool isNewChat = false)
        {
            ...
            var enumerator = GetGeneralReplyStreamInternalAsync(prompt, moduleContext, history).GetAsyncEnumerator();
            bool failed = false;
            ...
            if (failed)
            {
                await foreach (var part in _fallbackMock.GetGeneralReplyStreamAsync(prompt, moduleContext, history, false))
                {
                    yield return part;
                }
            }
...
6. AML TRACKER:
- WORKSPACE: Central queue for assigning, reviewing, and escalating active sanction cases.
- SLA LIMITS: High-risk cases have a 24-hour limit; Medium/Low have 48-72 hours.
- VIEWS: Customer 360, SLA Config, Reports, and Leaderboard.

7. INSTANT SANCTION SCREENING:
- LIGHTWEIGHT: Run ad-hoc checks against UAE local, UN, and IEC lists without creating workflow cases.
- SCORE: Phonetic match score > 80 indicates high risk requiring manual review.

8. COMPLIANCE HUB:
- REGULATIONS: Policy library containing Decree-Law No. 20 of 2018, UAE Central Bank rules, and FATF recommendations.
- SEARCH: Full-text search across policy documents.

9. SECTORAL TRANSACTION MONITORING (STM):
- SECTORS: Rules covering Insurance and Real Estate transaction patterns.
- e-FILING: Assisting in generating and e-filing STRs directly to Financial Intelligence Unit (FIU) via goAML portal.
- THRESHOLDS: Configurable rules (e.g., single transaction > 55,000 AED).

10. CASE SEARCH & FUZZY RESOLUTION FLOW:
- FUZZY NAME SEARCH: If the user queries case status, history, or next actions without a Case ID, the system searches cases by Customer Name fuzzy matching.
- RESOLUTION OPTIONS: If multiple cases are found, the Case IDs/Names are shown as selectable buttons, along with a manual input field to type a Case ID.
- ONE MATCH AUTO-SELECTION: If exactly one case is found, the system resolves it automatically and continues with the pending query.
";
        }
    }
}
