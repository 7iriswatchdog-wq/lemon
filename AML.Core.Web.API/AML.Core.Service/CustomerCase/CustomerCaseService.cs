    ...
    public class CustomerCaseService : BaseService, ICustomerCaseService
    {
        ...
        public string Create(CustomerCaseDTO _CustomerCaseDT, bool returnId = false)
        {
            CustomerMasterDTO custDetails = new CustomerMasterDTO()
            {
                ...
                IsDuplicate = _CustomerCaseDT.IsDuplicate,
                GroupId = _CustomerCaseDT.GroupId,
                GroupRisk = _CustomerCaseDT.GroupRisk,
                GroupEntityof = _CustomerCaseDT.GroupEntityof,
                //WhiteListed = _CustomerCaseDT.IsWhiteListed,
                //IsWhiteListed = _CustomerCaseDT.IsWhiteListed
            };
        ...
        public ServiceResponse<string> Create(CustomerCaseDTO _CustomerCaseDT)
        {
            ...
            CustomerMasterDTO custDetails = new CustomerMasterDTO()
            {
                ...
                Threshold = _CustomerCaseDT.Threshold,
                CompanyCode = _CustomerCaseDT.CompanyCode,
                CreatedOnDB = _CustomerCaseDT.CreatedOn.ParseDB() ?? DateTime.Now,
                Type = (_CustomerCaseDT.Type == "C" || _CustomerCaseDT.Type == "I") 
                       ? (_CustomerCaseDT.Type == "C" ? "Corporate" : "Individual") 
                       : _CustomerCaseDT.Type,
                ...
                EmiratesIdNumber=_CustomerCaseDT.EmiratesIdNumber,
                EmiratesIdExpiryDate=_CustomerCaseDT.EmiratesIdExpiryDate,
                EmiratesIdIssueDate=_CustomerCaseDT.EmiratesIdIssueDate,
                IsDuplicate=_CustomerCaseDT.IsDuplicate,
                Version=_CustomerCaseDT.Version,
                Tradelicense=_CustomerCaseDT.Tradelicense,
                GroupId=_CustomerCaseDT.GroupId,
                GroupRisk=_CustomerCaseDT.GroupRisk,
                GroupEntityof=_CustomerCaseDT.GroupEntityof,
                //WhiteListed = _CustomerCaseDT.IsWhiteListed,
                PlaceOfBirth=_CustomerCaseDT.PlaceOfBirth,
                
                //EstablishmentDate=_CustomerCaseDT.EstablishmentDate,
                //Address= _CustomerCaseDT.Address
                ReplaceFlag=_CustomerCaseDT.ReplaceFlag
            };
            ServiceResponse<string> _res = _customerMasterRepository.Create(custDetails);
            ...
            {
                if (_res.Result.Contains("Ø"))
                {
                    resu = _res.Result.Split('Ø')[0];
                    resu1 = _res.Result.Split('Ø')[1];
                }
                else if (_res.Result.Contains("??"))
                {
                    resu = _res.Result.Split(new string[] { "??" }, StringSplitOptions.None)[0];
                    resu1 = _res.Result.Split(new string[] { "??" }, StringSplitOptions.None)[1];
                }
                else if (_res.Result.Contains("A~"))
                {
                    resu = _res.Result.Split(new string[] { "A~" }, StringSplitOptions.None)[0];
                    resu1 = _res.Result.Split(new string[] { "A~" }, StringSplitOptions.None)[1];
                }
                else
                {
                    resu = _res.Result;
            ...
            if (resu != null && resu != "0")
            {
                // resu now contains only the primary key ID if the split worked
            ...
            if (resu1 != null && resu1 != "0" && resu != null && resu != "0")
            {
                ...
                // Ensure the new case is whitelisted for screening visibility in grids
                if (_CustomerCaseDT.IsWhiteListed == "YES")
                {
                    _customerMasterRepository.UpdateWhiteList(_CustomerCaseDT.CustomerMasterId, "YES");
                }
            }
        ...
        public ServiceResponse<string> CreatePrefix(CustomerCaseDTO _CustomerCaseDT)
        {
            ...
            if (custDetails.ScreeningOptions == "ALL")
            {
                var codes = GetCodesByClientID(custDetails.ClientId);
                if (codes != null && codes.Any())
                {
                    custDetails.ScreeningOptions = string.Join(", ", codes.Select(x => x.ccName));
                }
            }
            ...
            if (CustId.IsNotNullOrEmpty())
            {
                if (CustId.Contains("??"))
                {
                    var ids = CustId.Split(new string[] { "??" }, StringSplitOptions.None);
                    _CustomerCaseDT.CustomerMasterId = ids[0].ParseInt();
                    _CustomerCaseDT.CustomerId = ids[1];
                }
                else if (CustId.Contains("Ø"))
                {
                    var ids = CustId.Split('Ø');
                    _CustomerCaseDT.CustomerMasterId = ids[0].ParseInt();
                    _CustomerCaseDT.CustomerId = ids[1];
                }
                else
        ...
        public CustomerCaseDTO GetDetailsByVersion(int Id, int Version)
        {
            //Perform business requirements here
            return _CustomerCaseRepository.GetDetailsByVersion(Id,Version).Result;
        }
        public CustomerCaseDTO GetDetailsByCustidVersion(string Id, int Version)
        {
            //Perform business requirements here
            return _CustomerCaseRepository.GetDetailsByCustidVersion(Id, Version).Result;
        }
        ...
        public string GetMilitaryGoodsStatus(string customerId)
        {
            //Perform business requirements here
            return _CustomerCaseRepository.GetMilitaryGoodsStatus(customerId).Result;
        }
        public List<CustomerCaseDTO> GetShareHoldersByCompanyCode(string Id)
        {
            // 1. Get direct hierarchy members (via CompanyCode)
            var directMembers = _CustomerCaseRepository.GetShareHoldersByCompanyCode(Id).Result ?? new List<CustomerCaseDTO>();
            
            var parentEntity = _CustomerCaseRepository.GetCaseFullDetailsByCustomerId(Id).Result;
            int cId = parentEntity != null ? parentEntity.ClientId : 0;
            int uId = parentEntity != null ? parentEntity.CreatedBy : 0;
            var caseCreatedMembers = _CustomerCaseRepository.GetAllCaseCreatedShareHolders(cId, Id, uId).Result ?? new List<CustomerCaseDTO>();

            foreach (var member in caseCreatedMembers)
            {
                var existing = directMembers.FirstOrDefault(m => m.CustomerId == member.CustomerId);
                if (existing != null)
                {
                    // Member already in directMembers (from DB) — merge Id into its ParentID
                    // so it is visible as a child of this entity in the Related Parties tree.
                    var existingParents = (existing.ParentID ?? "")
                        .Split(',')
                        .Select(p => p.Trim())
                        .Where(p => !string.IsNullOrEmpty(p))
                        .ToList();
                    if (!existingParents.Contains(Id))
                    {
                        existingParents.Insert(0, Id);
                        existing.ParentID = string.Join(",", existingParents);
                    }
                    if (string.IsNullOrEmpty(existing.CompanyCode))
                    {
                        existing.CompanyCode = Id;
                    }
                }
                else
                {
                    member.CompanyCode = Id;
                    member.ParentID = Id;
                    directMembers.Add(member);
                }
            }

            // 1.5 Get immediate parent if this is a nested case/main entity
            if (parentEntity != null && !string.IsNullOrEmpty(parentEntity.ParentID) && parentEntity.ParentID != Id)
            {
                var immediateParent = _CustomerCaseRepository.GetCaseFullDetailsByCustomerId(parentEntity.ParentID).Result;
                if (immediateParent != null)
                {
                    if (!directMembers.Any(m => m.CustomerId == immediateParent.CustomerId))
                    {
                        directMembers.Add(immediateParent);
                    }
                }
            }

            // 1.6 Supplementary: fetch entities whose parent_id directly references this entity.
            // This catches multi-parent entities (e.g. ISH of KSE + AS of KE) where company_code
            // was overwritten by a later hierarchy save and no longer contains this entity's Id.
            try
            {
                var byParentMembers = _CustomerCaseRepository.GetShareHoldersByParentCode(Id).Result ?? new List<CustomerCaseDTO>();
                foreach (var member in byParentMembers)
                {
                    if (member.CustomerId == Id) continue;
                    var existing = directMembers.FirstOrDefault(m => m.CustomerId == member.CustomerId);
                    if (existing != null)
                    {
                        // Merge Id into existing ParentID so the view can find it as a child
                        var pts = (existing.ParentID ?? "").Split(',').Select(p => p.Trim()).Where(p => !string.IsNullOrEmpty(p)).ToList();
                        if (!pts.Contains(Id)) { pts.Insert(0, Id); existing.ParentID = string.Join(",", pts); }
                    }
                    else
                    {
                        // Add with ParentID = Id to ensure it's found as a child
                        var pts = (member.ParentID ?? "").Split(',').Select(p => p.Trim()).Where(p => !string.IsNullOrEmpty(p)).ToList();
                        if (!pts.Contains(Id)) pts.Insert(0, Id);
                        member.ParentID = string.Join(",", pts);
                        if (string.IsNullOrEmpty(member.CompanyCode)) member.CompanyCode = Id;
                        directMembers.Add(member);
                    }
                }
            }
            catch { /* non-critical — do not break main hierarchy fetch */ }

            // 1.7 Recursively fetch children/shareholders of any corporate nodes in the direct hierarchy (to handle nested main entities and their children)
            var corporateQueue = new Queue<string>(directMembers
                .Where(m => m.CustomerId != Id && (
                    m.CustomerType == "C" || 
                    m.CustomerType == "Corporate" || 
                    (m.CustomerType != null && m.CustomerType.StartsWith("Corporate_")) ||
                    m.Type == "Corporate" ||
                    (m.Type != null && m.Type.StartsWith("Corporate_"))
                ))
                .Select(m => m.CustomerId));
            var processedCorporates = new HashSet<string> { Id };

            while (corporateQueue.Count > 0)
            {
                var corpId = corporateQueue.Dequeue();
                if (processedCorporates.Contains(corpId)) continue;
                processedCorporates.Add(corpId);

                var children = _CustomerCaseRepository.GetShareHoldersByCompanyCode(corpId).Result;
                if (children != null)
                {
                    foreach (var child in children)
                    {
                        if (!directMembers.Any(m => m.CustomerId == child.CustomerId))
                        {
                            directMembers.Add(child);
                            if (child.CustomerType == "C" || 
                                child.CustomerType == "Corporate" || 
                                (child.CustomerType != null && child.CustomerType.StartsWith("Corporate_")) ||
                                child.Type == "Corporate" ||
                                (child.Type != null && child.Type.StartsWith("Corporate_")))
                            {
                                corporateQueue.Enqueue(child.CustomerId);
                            }
                        }
                    }
                }
            ...
            // 2. Identify all groups involved in this hierarchy, INCLUDING the parent entity itself
            var groupIds = directMembers
                .Where(m => !string.IsNullOrEmpty(m.GroupId))
                .SelectMany(m => m.GroupId.Split(',').Select(g => g.Trim()))
                .Where(g => !string.IsNullOrEmpty(g))
                .ToList();

            // Fetch the parent entity to get its groups as well
            if (parentEntity != null && !string.IsNullOrEmpty(parentEntity.GroupId))
            {
                var parentGroups = parentEntity.GroupId.Split(',').Select(g => g.Trim()).Where(g => !string.IsNullOrEmpty(g));
                groupIds.AddRange(parentGroups);
            }

            groupIds = groupIds.Distinct().ToList();
            
            // 3. Fetch all members of these groups and THEIR hierarchies
            var allMembers = new List<CustomerCaseDTO>(directMembers);
            var processedGroups = new HashSet<string>();
            var queue = new Queue<string>(groupIds);
            ...
            while (queue.Count > 0)
            {
                ...
                var groupMembers = _CustomerCaseRepository.GetCasesByGroupId(groupId).Result;
                if (groupMembers != null)
                {
                    foreach (var member in groupMembers)
                    {
                        var existing = allMembers.FirstOrDefault(m => m.CustomerId == member.CustomerId);
                        if (existing != null)
                        {
                            if (string.IsNullOrEmpty(existing.GroupId))
                            {
                                existing.GroupId = member.GroupId;
                                existing.GroupRisk = member.GroupRisk;
                                existing.GroupEntityof = member.GroupEntityof;
                            }
                        }
                        
                        if (!allMembers.Any(m => m.CustomerId == member.CustomerId))
                        {
                            allMembers.Add(member);
                            
                            // Recursively add any new groups found in group members
                            if (!string.IsNullOrEmpty(member.GroupId))
                            {
                                var subGroups = member.GroupId.Split(',').Select(g => g.Trim()).Where(g => !string.IsNullOrEmpty(g));
                                foreach (var sg in subGroups) queue.Enqueue(sg);
                            }

                            // Fetch the hierarchy of this group member to include its children
                            var memberHierarchy = _CustomerCaseRepository.GetShareHoldersByCompanyCode(member.CustomerId).Result;
                            if (memberHierarchy != null)
                            {
                                foreach (var hMember in memberHierarchy)
                                {
                                    var existingH = allMembers.FirstOrDefault(m => m.CustomerId == hMember.CustomerId);
                                    if (existingH != null)
                                    {
                                        if (string.IsNullOrEmpty(existingH.GroupId))
                                        {
                                            existingH.GroupId = hMember.GroupId;
                                            existingH.GroupRisk = hMember.GroupRisk;
                                            existingH.GroupEntityof = hMember.GroupEntityof;
                                        }
                                    }
                                    
                                    if (!allMembers.Any(m => m.CustomerId == hMember.CustomerId))
                                    {
                                        allMembers.Add(hMember);
                                        
                                        // Also check for groups in hierarchy members
                                        if (!string.IsNullOrEmpty(hMember.GroupId))
                                        {
                                            var hGroups = hMember.GroupId.Split(',').Select(g => g.Trim()).Where(g => !string.IsNullOrEmpty(g));
                                            foreach (var hg in hGroups) queue.Enqueue(hg);
                                        }
                                    }
                                }
                            }
        ...
        public List<CustomerCaseDTO> GetAllVersionCases(string Id)
        {
            //Perform business requirements here
            return _CustomerCaseRepository.GetAllVersionCases(Id).Result;
        }
        ...
        public int GetCaseIdByVersion(string custId, int version)
        {
            return _CustomerCaseRepository.GetCaseIdByVersion(custId, version);
        }
        ...
        public List<CustomerCaseDTO> GetAll(int userId, string startDate, string endDate, string cust_type, string matchscore, int createdBy,int caseStatus,string riskLevel,  string usergroupName,int clientId, string includingDuplicate)
        {
            //Perform business requirements here
            return _CustomerCaseRepository.GetAll(userId, startDate, endDate, cust_type, matchscore, createdBy, caseStatus, riskLevel, usergroupName, clientId,includingDuplicate).Result;
        }
        public List<CustomerCaseDTO> GetAllCompletedCases(int userId, string startDate, string endDate, string cust_type, string matchscore, int createdBy, int caseStatus, string riskLevel, int clientId, string includingDuplicate)
        {
            //Perform business requirements here
            return _CustomerCaseRepository.GetAllCompletedCases(userId, startDate, endDate, cust_type, matchscore, createdBy, caseStatus, riskLevel,clientId, includingDuplicate).Result;
        }
        public List<CustomerCaseDTO> GetAllBySearchValue(int userId, string startDate, string endDate, string cust_type,string searchValue, string matchscore, int createdBy, int caseStatus, string riskLevel, string usergroupName, int clientId, string includingDuplicate)
        {
            //Perform business requirements here
            return _CustomerCaseRepository.GetAllBySearchValue(userId, startDate, endDate, cust_type,searchValue, matchscore, createdBy, caseStatus, riskLevel, usergroupName,clientId, includingDuplicate).Result;
        }
        public List<CustomerCaseDTO> GetAllCompletedBySearchValue(int userId, string startDate, string endDate, string cust_type, string searchValue, string matchscore, int createdBy, int caseStatus, string riskLevel, int clientId, string includingDuplicate)
        {
            //Perform business requirements here
            return _CustomerCaseRepository.GetAllCompletedBySearchValue(userId, startDate, endDate, cust_type, searchValue, matchscore, createdBy, caseStatus, riskLevel,clientId,  includingDuplicate).Result;
        }
        ...
        {
            //Perform business requirements here
            var res = _CustomerCaseRepository.GetAllCaseCreatedShareHolders(clientid, companyCode, userId).Result ?? new List<CustomerCaseDTO>();
            foreach (var m in res)
            {
                m.CompanyCode = companyCode;
                m.ParentID = companyCode;
            }
            return res;
        }
        ...
        public List<CustomerCaseDTO> GetCasesByGroupId(string groupId)
        {
            return _CustomerCaseRepository.GetCasesByGroupId(groupId).Result;
        }
    }
}
