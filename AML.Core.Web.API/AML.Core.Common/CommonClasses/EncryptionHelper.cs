...
namespace AML.Core.Common.CommonClasses
{
    public class EncryptionHelper
    {
        ...
        public static string EncryptFileUrl(string image_path)
        {
            byte[] b = System.Text.ASCIIEncoding.ASCII.GetBytes(image_path);
            string encrypted = Convert.ToBase64String(b);
            return encrypted;
        }

        public static byte[] DecodeUrlBase64(string path)
        {
            path = path.Replace('-', '+').Replace('_', '/').PadRight(4 * ((path.Length + 3) / 4), '=');
